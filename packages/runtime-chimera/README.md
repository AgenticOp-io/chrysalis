...
`chimera.json` (all fields optional; flags override file values). **Versioned (V2-M5, DESIGN D253):** set **`"kind": "chrysalis.chimera.config"`** and **`"schemaVersion": 1`** so every node can load the same file and the CLI rejects unknown contract versions. Omit **`kind`** for legacy implicit v0 files (still supported). **Optional HMAC (D255 / D257):** **`hmacSha256`** may be one **64-hex string** or an **object** of **`keyId → hex`** digests; verify with **`CHRYSALIS_CHIMERA_CONFIG_HMAC_SECRET`**, optional **`CHRYSALIS_CHIMERA_CONFIG_HMAC_PREVIOUS_SECRETS`**, or **`CHRYSALIS_CHIMERA_CONFIG_HMAC_KEYS_JSON`** / **`--config-hmac-keys-json`**. Use **`computeChimeraDeployConfigHmacHex`**, **`computeChimeraDeployConfigHmacHexByKeyIds`**, and **`stableStringifyChimeraDeploySigningPayload`** to generate signatures. **Central URL + reload (D256):** **`chrysalis deploy --config-url …`** or **`CHRYSALIS_CHIMERA_CONFIG_URL`**; send **`SIGHUP`** or **`SIGUSR2`** to the deploy process to re-fetch/re-read JSON and restart the in-process chimera (brief local blip; failures keep the prior server). **Operator drift (D258):** **`buildChimeraOperatorSnapshot`**, **`computeChimeraDeployRoutingFingerprintSha256`**; CLI **`--operator-metrics-json`**, **`--operator-metrics-ndjson`**, **`CHRYSALIS_CHIMERA_INSTANCE_ID`**. **Fleet batch (D259):** NDJSON lines of **`chrysalis.chimera.operator-snapshot`** merge with **`scripts/aggregate-chimera-operator-snapshots.mjs`** into **`chrysalis.chimera.operator-snapshot.batch`** (**`CHIMERA_OPERATOR_SNAPSHOT_BATCH_KIND`**, **`CHIMERA_OPERATOR_SNAPSHOT_BATCH_SCHEMA_VERSION`**).
