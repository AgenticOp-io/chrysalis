...
- `chrysalis convert` — same ingest + emit pipeline as **`emit`** today (Milestone 1 convenience); forwards the same **`--ingest-*`** / **`--emit-*`** / shard flags, including **`--ingest-dedupe-structural-subgraphs`** (**D283**)
