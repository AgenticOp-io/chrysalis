...
describe("emit-hono: flagship laravel-full chrysalis-templates", () => {
  test("emits fifty-three handlers and zero holes", async () => {
    ...
    try {
      ...
      expect(res.handlerCount).toBe(53);
      expect(res.holes.length).toBe(0);
...
describe("emit-hono: emitDedupeIdenticalHandlerBodies", () => {
  ...
  test("dedupe with emitRoutePathConstants: server uses ChrysalisRoutePaths; one deduped module", async () => {
    const out = mkdtempSync(resolve(tmpdir(), "chrysalis-emit-dedupe-rpc-"));
    try {
      const mod = moduleTwoRoutesIdenticalProbeHoles();
      await emit({
        module: mod,
      ...
      expect(paths).toContain("handler_b");
      const serverTs = readFileSync(resolve(out, "src/server.ts"), "utf8");
      expect(serverTs).toContain("ChrysalisRoutePaths");
      expect(serverTs).toContain("./chrysalis-route-paths.js");
  ...
  test("dedupe with runtimeFacadeModule: shared body imports runtime via facade", async () => {
    const out = mkdtempSync(resolve(tmpdir(), "chrysalis-emit-dedupe-facade-"));
    try {
      const mod = moduleTwoRoutesIdenticalProbeHoles();
      await emit({
        module: mod,
  ...
  test("dedupe with emitHandlerFingerprints: JSON lists distinct thin-wrapper shas", async () => {
    const out = mkdtempSync(resolve(tmpdir(), "chrysalis-emit-dedupe-fp-"));
    try {
      const mod = moduleTwoRoutesIdenticalProbeHoles();
      await emit({
        module: mod,
  ...
  test("dedupe with emitSharedRuntimeImports: thin handlers and shared body import chrysalis-runtime-imports", async () => {
    const out = mkdtempSync(resolve(tmpdir(), "chrysalis-emit-dedupe-sri-"));
    try {
      const mod = moduleTwoRoutesIdenticalProbeHoles();
      await emit({
        module: mod,
  ...
  test("dedupe with emitSharedRuntimeImports and runtimeFacadeModule: SRI re-exports facade; deduped body uses SRI", async () => {
    const out = mkdtempSync(resolve(tmpdir(), "chrysalis-emit-dedupe-sri-facade-"));
    try {
      const mod = moduleTwoRoutesIdenticalProbeHoles();
      await emit({
        module: mod,
...
describe("emit-hono: lib-helpers module (G2318/G2321/G2326)", () => {
