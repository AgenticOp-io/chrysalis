...
describe("tryExtractInlineQuery (G2334)", () => {
  ...
  it("accepts strripos(, literal) wrapper on formal assign (G7090)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    ...
    const assignVal = data.call({
      ...
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "strripos", args: [label, comma], type: T.int, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrriposFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: comma });
  });

  it("accepts str_contains(, literal) wrapper on formal assign (G7091)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    ...
    const assignVal = data.call({
      ...
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "str_contains", args: [label, comma], type: T.bool, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrContainsFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: comma });
  });

  it("accepts str_starts_with(, literal) wrapper on formal assign (G7092)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    const label = data.param({ name: "label", type: T.string, origin });
    const prefix = data.literal({ value: "A", type: T.string, origin });
    const assignVal = data.call({
      callee: "__assign",
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "str_starts_with", args: [label, prefix], type: T.bool, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrStartsWithFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: prefix });
  });

  it("accepts str_ends_with(, literal) wrapper on formal assign (G7093)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    const label = data.param({ name: "label", type: T.string, origin });
    const suffix = data.literal({ value: "C", type: T.string, origin });
    const assignVal = data.call({
      callee: "__assign",
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "str_ends_with", args: [label, suffix], type: T.bool, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrEndsWithFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: suffix });
  });

  it("accepts substr_count(, literal) wrapper on formal assign (G7094)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    const label = data.param({ name: "label", type: T.string, origin });
    const needle = data.literal({ value: ",", type: T.string, origin });
    const assignVal = data.call({
      callee: "__assign",
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "substr_count", args: [label, needle], type: T.int, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToSubstrCountFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: needle });
  });

  it("accepts explode(, literal) wrapper on formal assign (G7095)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    const label = data.param({ name: "label", type: T.string, origin });
    const sep = data.literal({ value: ",", type: T.string, origin });
    const assignVal = data.call({
      callee: "__assign",
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "explode", args: [sep, label], type: T.array(T.string), origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToExplodeFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: sep });
  });

  it("accepts strcmp(, literal) wrapper on formal assign (G7096)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    const label = data.param({ name: "label", type: T.string, origin });
    const other = data.literal({ value: "A", type: T.string, origin });
    const assignVal = data.call({
      callee: "__assign",
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "strcmp", args: [label, other], type: T.int, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrcmpFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: other });
  });

  it("accepts strcasecmp(, literal) wrapper on formal assign (G7097)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    ...
    const assignVal = data.call({
      ...
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "strcasecmp", args: [label, other], type: T.int, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrcasecmpFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: other });
  });

  it("accepts strncmp(, literal, literal) wrapper on formal assign (G7098)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    ...
    const assignVal = data.call({
      ...
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "strncmp", args: [label, needle, length], type: T.int, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrncmpFormalLiteral2.get("$v")).toEqual({
      formal: "label",
  ...
  it("accepts ucfirst() wrapper on formal assign (B57/G7107)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    ...
    const assignVal = data.call({
      ...
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "ucfirst", args: [label], type: T.string, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToUcfirstFormal.get("$v")).toBe("label");
  });

  it("accepts str_split(, literal) wrapper on formal assign (B65/G7117)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    const label = data.param({ name: "label", type: T.string, origin });
    const length = data.literal({ value: 2, type: T.int, origin });
    const assignVal = data.call({
      callee: "__assign",
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "str_split", args: [label, length], type: T.array(T.string), origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrSplitFormalLiteral.get("$v")).toEqual({ formal: "label", literalId: length });
  });

  it("accepts str_replace(, literal, literal) wrapper on formal assign (B55/G7105)", () => {
    const builder = new ModuleBuilder({ sourceApp: "test", chrysalisVersion: "1.0.0" });
    const data = dataDialect.builders(builder);
    const effect = effectDialect.builders(builder);
    const origin = phpLocator("lib.php", 1, 1);
    const label = data.param({ name: "label", type: T.string, origin });
    const search = data.literal({ value: "a", type: T.string, origin });
    const replace = data.literal({ value: "b", type: T.string, origin });
    const assignVal = data.call({
      callee: "__assign",
      args: [
        data.literal({ value: "v", type: T.string, origin }),
        data.call({ callee: "str_replace", args: [label, search, replace], type: T.string, origin }),
      ],
      type: T.void,
      origin,
    });
    ...
    const mod = builder.finish();
    const extracted = tryExtractInlineQuery(mod, body, ["label"]);
    expect(extracted!.localToStrReplaceFormalLiteral2.get("$v")).toEqual({
      formal: "label",
      searchLiteralId: search,
      replaceLiteralId: replace,
    });
  });
});
