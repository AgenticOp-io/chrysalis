...
- **No-source markup holes (D6369 / G9480):** **`applyNoSourceMarkupHolesToCwlSource`**, **`buildNoSourceMarkupHoleHtml`**, **`MARKUP_NO_SOURCE_HOLE_REASON`** — replace demo shells for routes without source markup with explicit `legacy:markup-no-source-route` holes.
