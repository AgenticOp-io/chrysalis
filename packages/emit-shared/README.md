...
- **UI route stylesheets (D6365 / D6368):** **`resolveRouteStylesheetHref`**, **`resolveRouteStylesheetHrefs`** (route + fallback), **`routeStylesheetLinkTag`**, **`wrapHtmlFragmentWithDocumentShell`**, **`loadUiAssetLiftArtifacts`** — backend-agnostic consumption of `chrysalis.ui.route-style-map` (**G9300c** / **G9470**).
- **UI route markup (G9308–G9309):** **`resolveRouteMarkupHref`**, **`findRouteMarkupBundle`**, **`applyLiftedMarkupToCwlSource`**, **`loadUiMarkupLiftArtifacts`**.
- **No-source markup holes / form shells (D6369 / G9480 / G9790):** **`applyNoSourceMarkupHolesToCwlSource`**, **`buildNoSourceMarkupHoleHtml`**, **`buildNoSourceFormShellHtml`**, **`MARKUP_NO_SOURCE_HOLE_REASON`** — replace demo shells for routes without source markup with explicit holes or empty form chrome (`formShell: true`; no invented fields).
