...
describe("emit-fastify: emitDedupeIdenticalHandlerBodies", () => {
  ...
  test("dedupe with handlerImportBarrel: thin handlers use barrel; shared dedupe module uses direct fastify", async () => {
    const out = mkdtempSync(resolve(tmpdir(), "chrysalis-emit-f-dedupe-barrel-"));
    try {
      const mod = moduleTwoRoutesIdenticalProbeHoles();
      await emit({
        module: mod,
      ...
      expect(shared).toContain('from "fastify"');
  ...
  test("dedupe with routeRegistration lazy: server uses dynamic import; dedupe unchanged", async () => {
    const out = mkdtempSync(resolve(tmpdir(), "chrysalis-emit-f-dedupe-lazy-"));
    try {
      const mod = moduleTwoRoutesIdenticalProbeHoles();
      await emit({
        module: mod,
      ...
      const serverTs = readFileSync(resolve(out, "src/server.ts"), "utf8");
      expect(serverTs).toContain('await import("./handlers/');
      expect(serverTs).toContain("handler_a");
      const dedupeDir = resolve(out, "src/chrysalis-deduped");
      expect(readdirSync(dedupeDir).length).toBe(1);
    } finally {
