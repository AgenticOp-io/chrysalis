...
- **Flagship parity:** Same Vitest slices as emit-hono — **`flagship/laravel-min`**
  ...
  (thirty-seven handlers: `ping_show.ts`, `health_txt_show.ts`, `api_health_show.ts`, `jump_show.ts`,
  ...
  `between_count_show.ts`, `eq_one_count_show.ts`, `eq_three_count_show.ts`, `eq_two_count_show.ts`, `ne_one_count_show.ts`, `ne_three_count_show.ts`, `lt_two_count_show.ts`, `gt_one_count_show.ts`, `session_login_post.ts`, `session_logout_post.ts`, `echo_post.ts`, zero holes).
