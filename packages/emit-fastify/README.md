...
- **Flagship parity:** Same Vitest slices as emit-hono — **`flagship/laravel-min`**
  ...
  (twenty-five handlers: `ping_show.ts`, `health_txt_show.ts`, `api_health_show.ts`, `jump_show.ts`,
  `count_show.ts`, `framework_show.ts`, `first_item_show.ts`, `last_item_show.ts`, `items_list_show.ts`, `lib_count_show.ts`, `sum_ids_show.ts`, `min_id_show.ts`, `max_id_show.ts`, `avg_id_show.ts`, `id_span_show.ts`, `sum_squares_show.ts`, `even_count_show.ts`, `odd_count_show.ts`, `gt_two_count_show.ts`, `session_visit_show.ts`, `session_me_show.ts`, `hello_show.ts`,
