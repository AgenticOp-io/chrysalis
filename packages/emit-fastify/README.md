...
- **Flagship parity:** Same Vitest slices as emit-hono — **`flagship/laravel-min`**
  ...
  (fourteen handlers: `ping_show.ts`, `health_txt_show.ts`, `api_health_show.ts`, `jump_show.ts`,
  `count_show.ts`, `framework_show.ts`, `first_item_show.ts`, `items_list_show.ts`, `session_visit_show.ts`, `session_me_show.ts`, `hello_show.ts`,
