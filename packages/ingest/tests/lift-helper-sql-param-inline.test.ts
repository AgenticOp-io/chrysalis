...
describe("ingest: lift-helper-sql-param-inline (B5.5 v3+)", () => {
  ...
  it("prelude guard allows strlen skip but blocks sideeffect pre-return query", async () => {
    ...
    const sha1Helper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_sha1")!;
    expect(tryExtractInlineQuery(mod, sha1Helper.bodyId, sha1Helper.paramNames)).toBeDefined();
    const base64_encodeHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_base64_encode")!;
    expect(tryExtractInlineQuery(mod, base64_encodeHelper.bodyId, base64_encodeHelper.paramNames)).toBeDefined();
    const base64_decodeHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_base64_decode")!;
    expect(tryExtractInlineQuery(mod, base64_decodeHelper.bodyId, base64_decodeHelper.paramNames)).toBeDefined();
    const bin2hexHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_bin2hex")!;
    expect(tryExtractInlineQuery(mod, bin2hexHelper.bodyId, bin2hexHelper.paramNames)).toBeDefined();
    const preg_quoteHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_preg_quote")!;
    expect(tryExtractInlineQuery(mod, preg_quoteHelper.bodyId, preg_quoteHelper.paramNames)).toBeDefined();
    const parse_urlHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_parse_url")!;
    expect(tryExtractInlineQuery(mod, parse_urlHelper.bodyId, parse_urlHelper.paramNames)).toBeDefined();
    const basenameHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_basename")!;
    expect(tryExtractInlineQuery(mod, basenameHelper.bodyId, basenameHelper.paramNames)).toBeDefined();
    const dirnameHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_dirname")!;
    expect(tryExtractInlineQuery(mod, dirnameHelper.bodyId, dirnameHelper.paramNames)).toBeDefined();
    const gettypeHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_gettype")!;
    expect(tryExtractInlineQuery(mod, gettypeHelper.bodyId, gettypeHelper.paramNames)).toBeDefined();
    const is_callableHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_is_callable")!;
    expect(tryExtractInlineQuery(mod, is_callableHelper.bodyId, is_callableHelper.paramNames)).toBeDefined();
    const is_resourceHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_is_resource")!;
    expect(tryExtractInlineQuery(mod, is_resourceHelper.bodyId, is_resourceHelper.paramNames)).toBeDefined();
    const ordHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_ord")!;
    expect(tryExtractInlineQuery(mod, ordHelper.bodyId, ordHelper.paramNames)).toBeDefined();
    const chrHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_chr")!;
    expect(tryExtractInlineQuery(mod, chrHelper.bodyId, chrHelper.paramNames)).toBeDefined();
    const preg_matchHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_preg_match")!;
    expect(tryExtractInlineQuery(mod, preg_matchHelper.bodyId, preg_matchHelper.paramNames)).toBeDefined();
    const hashHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_hash")!;
    expect(tryExtractInlineQuery(mod, hashHelper.bodyId, hashHelper.paramNames)).toBeDefined();
    const sprintfHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_sprintf")!;
    expect(tryExtractInlineQuery(mod, sprintfHelper.bodyId, sprintfHelper.paramNames)).toBeDefined();
    const number_format2Helper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_number_format2")!;
    expect(tryExtractInlineQuery(mod, number_format2Helper.bodyId, number_format2Helper.paramNames)).toBeDefined();
    const implodeHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_implode")!;
    expect(tryExtractInlineQuery(mod, implodeHelper.bodyId, implodeHelper.paramNames)).toBeDefined();
    const preg_replaceHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_preg_replace")!;
    expect(tryExtractInlineQuery(mod, preg_replaceHelper.bodyId, preg_replaceHelper.paramNames)).toBeDefined();
    const preg_splitHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_preg_split")!;
    expect(tryExtractInlineQuery(mod, preg_splitHelper.bodyId, preg_splitHelper.paramNames)).toBeDefined();
    const hexdecHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_hexdec")!;
    expect(tryExtractInlineQuery(mod, hexdecHelper.bodyId, hexdecHelper.paramNames)).toBeDefined();
    const dechexHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_dechex")!;
    expect(tryExtractInlineQuery(mod, dechexHelper.bodyId, dechexHelper.paramNames)).toBeDefined();
    const filter_varHelper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_filter_var")!;
    expect(tryExtractInlineQuery(mod, filter_varHelper.bodyId, filter_varHelper.paramNames)).toBeDefined();
    const crc32Helper = resolveHelperBodyEntry(bodies, "chrysalis_sql_param_crc32")!;
    expect(tryExtractInlineQuery(mod, crc32Helper.bodyId, crc32Helper.paramNames)).toBeDefined();
