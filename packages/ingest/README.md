...
- `IngestOptions.dedupeStructuralSubgraphs` (opt-in, **DESIGN D283**) — after lowering, run **`dedupeStructuralSubgraphsInModule`** from **`@chrysalis/webir`** (same structural key as **`mergeWebIrModules`**, **D247**) on the monolithic module. CLI **`--ingest-dedupe-structural-subgraphs`** on ingest-driven commands. Default **off**; when on, **`Module.nodes`** may shrink vs default ingest for identical subgraphs.
