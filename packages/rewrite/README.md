...
| `batch-n1-read` | `n-plus-one-queries` | Requires the foreach iterable to be a `data.param`, each batched inner read to be `returns: row-or-null` with one `?` param from `member(loopVar, fkColumn)`, SQL `SELECT cols FROM tbl WHERE col = ?` or `SELECT *` (projected via the FK column), and the inner `effect.db.query` to be the RHS of `__assign`. Batches **every** such inner read in loop order (disambiguated temp names when there are several). Listed in `DEFAULT_PASSES` after `dispatch-union-zod`. See DESIGN.md D42/D43. |
