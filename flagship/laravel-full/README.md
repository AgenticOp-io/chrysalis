...
**Milestone 6A:** once scaffolded, **`pnpm run verify:laravel-full`** applies **`VERIFY_THRESHOLD`** to the full corpus and to the Chrysalis **session auth slice** (`GET/POST …/chrysalis-session/login`, `GET …/me`, `GET/POST …/logout`, **`GET /chrysalis-auth-probe`**, **`GET /chrysalis-socialite-fortify-probe`**) via **`scripts/milestone-6a-auth-verify-gate.mjs`** (D188, D190, D192). The **auth-probe** route returns JSON from stub **Laravel\Sanctum** and **League\OAuth2\Client** class names; **socialite-fortify-probe** uses **Laravel\Socialite** and **Laravel\Fortify** stub **`::probe()`** methods. See **`flagship/laravel-min/README.md`** for the complementary `Gate` + session login oracle.
...
- **`chrysalis.routes.json`** — fifty-three routes:
  ...
  **`GET /chrysalis-auth-probe`** (D190), **`GET /chrysalis-socialite-fortify-probe`** (D192),
