...
- **`chrysalis.routes.json`** — thirteen routes:
  ...
  **`POST /chrysalis-echo`**
  ...
  `count_show.php`, `framework_show.php`, `first_item_show.php`, `session_visit_show.php`, `session_me_show.php`, `hello_show.php`,
  `session_login_post.php`, `session_logout_post.php`, `echo_post.php`
- **`chrysalis/handlers/*.php`** — deterministic plain text / JSON / redirect / session / query surfaces
  (no `Date.now`, no env)
...
1. **`php artisan serve`** (or your stack) and open:
   ...
   **`/chrysalis-jump`**, **`/chrysalis-count`**, **`/chrysalis-framework`**, **`/chrysalis-first-item`**, **`/chrysalis-session/visit`**, **`/chrysalis-session/me`**,
...
4. After **`pnpm run build`** at the repo root, run **`pnpm run verify:laravel-full`**
   ...
   **`/chrysalis-jump`** (manual redirect), **`/chrysalis-count`** x2, **`/chrysalis-framework`** x2, **`/chrysalis-first-item`** x2, **`/chrysalis-session/visit`** x2,
