...
`GET/POST /login`, `POST /logout`, and `GET /session/me`, plus **`GET /gate-probe`** (stub
`Illuminate\Support\Facades\Gate::{allows,denies}` in `lib/gate_facade_stub.php`, D190). CI applies
`VERIFY_THRESHOLD` to the **full** replay report and to a **fixed auth-route subset**
(`milestone-6a-auth-verify-gate.mjs`, D188). Full Sanctum/MFA/OAuth translation is **not** claimed
for the **Laravel app** here; that path is exercised in **`../laravel-full/chrysalis-templates`**
via **`GET /chrysalis-auth-probe`**.
...
**`POST /login`** **403** on bad **CSRF**, **401** on wrong password, **400** on empty credentials, then the good login.
