...
- **`laravel-full/`** — Composer Laravel **adoption track** (README, empty manifest
  example, **`chrysalis-templates/`** ingest slice for ping/health/json/redirect/DB-count/framework-wrapper/DB-first-item/DB-items-list/session `visit/me/login/logout`, query, and POST routes; no
