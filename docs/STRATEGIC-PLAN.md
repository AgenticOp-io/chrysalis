...
1. **Maintain G9990** â€” `hub:ut-canon-program-close-smoke` + **G9991** `hub:ut-maintain-packaging-smoke` (**D6440**)
2. **G8550 composite** â€” `hub:migration-os-close-smoke`
3. **G7690 UT regression** â€” `hub:cwl-universal-translator-close-smoke`
4. **Origin gold only where a customer/chartered UT edge fails**
5. **Optional POC** â€” WISP / management.wisptools.io only if explicitly requested â†’ **G9992** `hub:wisp-poc-from-scratch` ([`WISP-POC-FROM-SCRATCH.md`](./WISP-POC-FROM-SCRATCH.md)) â€” still **true conversion only** (**D6442**/**D6447**)
