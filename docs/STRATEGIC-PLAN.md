...
**Amended 2026-07-27 (D6538 / G10077) — Tier A+ COBOL-primary queue hygiene.** Secondary-dialect / prefix bingo is **paused as the default “build” path**. When the operator says build without specifying in this lane: prefer **COBOL** (`hub:cobol-clbs-prove-smoke`, chartered Tier B Medium+ / Tier C only) over another Fiber/Hono/Gin peel. Reopen dialect peels only when explicitly asked. Scoreboard: [`LEADERSHIP-SCOREBOARD.md`](./LEADERSHIP-SCOREBOARD.md).
...
1. **COBOL primary** — prove / chartered Tier B Medium+ or Tier C only (`docs/COBOL-MODERNIZATION-PROVE.md`); **not** another secondary-dialect wave (G10077)
2. **Maintain G9990** â€” `hub:ut-canon-program-close-smoke` + **G9991** `hub:ut-maintain-packaging-smoke` (**D6440**)
3. **G8550 composite** â€” `hub:migration-os-close-smoke`
4. **G7690 UT regression** â€” `hub:cwl-universal-translator-close-smoke`
5. **Origin gold only where a customer/chartered UT edge fails**
6. **Optional POC** â€” WISP / management.wisptools.io only if explicitly requested â†’ **G9992** `hub:wisp-poc-from-scratch` ([`WISP-POC-FROM-SCRATCH.md`](./WISP-POC-FROM-SCRATCH.md)) â€” still **true conversion only** (**D6442**/**D6447**)
...
**Amended 2026-07-27 (G10065) â€” Revel Go secondary skipped.** `conf/routes` `METHOD PATH Controller.Action` + `func (c App) Action() revel.Result` is **not** Gin/Buffalo/Martini/Beego-functional peel-reuse (Rails-class; G10006 parallel). `conf/routes` is not `.go` so Go lift never sees the table; Action methods have no `.GET`/`.Get("/path"` registration. Probe: real routes lifted **0** (Params.*.Get false-positive Chi `.Get` matches only). Honest skip: [`fixtures/ci/revel-honest-skip.json`](../fixtures/ci/revel-honest-skip.json). No D6527. Gin remains Go D6448-ST; Chi/Echo/Fiber/Iris/Beego/Buffalo/Martini/Gorilla/ServeMux stay green. Catalog: `fixtures/ci/go-secondary-dialect-honest-holes.json`.
