...
**G9500 / D6371:** Fill every *fillable* hole — balanced `{#if}`/`{#each}`, showcase `loadBools`, static component inline, load-scalar hydration. Gate: `hub:wisp-fill-holes-smoke`. **G9790 / D6400:** no-source `/add` → empty form shells. **G9800 / D6401:** force-settle residual opaque holes to **0** (empty/omit, not invented widgets). GenieACS never in scope.
...
pnpm run hub:wisp-showcase-bound-smoke    # zero-hole bound after G9800
