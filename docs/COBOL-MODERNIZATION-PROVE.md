...
- **WebIR deepen (**G10085**–**G10092**):** `buildCobolWebIrHoleAttrs` attaches inventory catalogs to `data.hole` attrs; proven `detectEmitPattern` expecteds lower onto MAIN/sole-entry WebIR literals (CLBSMATH / CKPRSTRN / SEQSUM / EMPPAYRN / COPY-linked literals); arithmetic kinds also emit typed `data.binOp` (`*`/`/`/`+`) while retaining the formatted expected; **G10092** adds `seq-max` (operands + typed int max) and `perform-varying-sum` (`+` fold); `expandCobolCopybooks` inlines resolved in-repo `.cpy` and expands licensed DFHAID/DFHBMSCA **only when present on disk** (else skip; never invent); **G10089** COBOL site-inventory adapter; **G10090** residual ledger (P0 closes when licensed drop present). **IBM SDFHCOB:** [`COBOL-IBM-SDFHCOB-DROP.md`](./COBOL-IBM-SDFHCOB-DROP.md) — ZD&T ADCD `DFH550.CICS.SDFHCOB` on `B5C551`; local `DFHAID.cpy`/`DFHBMSCA.cpy` **gitignored**. Pattern-emit sidecar remains for cobc∥ multi-lang.
...
- **Tier C AID catalog (**G10084**):** `cicsAidSymbols` / `bmsAttrSymbols` from real upstream (DFHENTER/DFHPF*/DFHBMPRF/…); licensed local DFHAID/DFHBMSCA close residual when present; `EXTFMAP` / `CMQ*` stay unresolved without entitlement.
