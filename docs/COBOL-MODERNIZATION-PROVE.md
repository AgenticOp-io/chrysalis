...
- **WebIR deepen (**G10085**–**G10099**):** emit→WebIR catalogs through card-*; hole-attr control surface; indexed `limit`; `EXEC SQL INCLUDE` expand-when-present; **G10099** CICS `FILE`/`QUEUE` literals + BMS MAP/MAPSET crosswalk attrs on online holes (no VSAM/TDQ invent). **EXTFMAP** sole open P0. **IBM SDFHCOB:** [`COBOL-IBM-SDFHCOB-DROP.md`](./COBOL-IBM-SDFHCOB-DROP.md).
