...
| **Behavioral Fidelity** | 50 | GnuCOBOL vs emitted Python **per subject** (61 subjects incl. **histldrn**, **idxprobe**, **cardaccf**, **rptposrn**, **idxaltrn**, **cardschd**, **rptaurn**, **idxstrwr**, **rptstarn**, **prcseqrn**, **idxdelrn**, **idxaltrw**, **rtnanarn**, **bchctlrn**, **posupdrn**, **rcvprcrn**, **rtncdern**, **utlmntrn**, **utlvalrn**, **idxgtnrn**, **utlmonrn**, **tstvalrn**, **portvalrn**, **idxnlprn**, **utlmntls**, **tstgenrn**, **portaddrn**, **portupdrn**, **idxltprn**, **portdelrn**, **portreadrn**, **porttranrn**, **idxeqprn**, **portvaldn**, **idxngtrn**, **portmstrn**, **portcomrn**, **idxeqnrn**, **idxnlnrn**, **idxltnrn**, **idxngprn**); Java/C# `EXPECTED:` tags; **plus** `cobol-pattern-emit` generated contracts; or skip with `behavioralSkipped: true` when no compiler |
...
| `online/COTRTLIC.cbl` + `online/COTRTUPC.cbl` | CardDemo Db2 transaction-type **programs** (not maps): EXEC SQL cursor/DML + EXEC CICS catalog floors; COPY COTRTLI/COTRTUP resolve; **INCLUDE** CSDB2RWY/DCLTRTYP/CSDB2RPY (list) + DCLTRTYP/DCLTRCAT (update) resolve under `copybook/`; COTRTUPC also resolves CVCRD01Y/CSMSG02Y/CSSTRPFY and catalogs CSUTLDWY/CSSETATY as unresolved COPY holes; DFHAID+DFHBMSCA BMS holes (prove `online-cotrtlic-sql-cics-holes` / `online-cotrtupc-sql-cics-holes`). Upstream `_upstream/COTRTLIC.cbl` / `COTRTUPC.cbl` + CSDB2*/DCLTR*. |
| `online/COACTUPC.cbl` + `copybook/CVCUS01Y.cpy` | CardDemo account-update **program** (not map-only): EXEC CICS READ/REWRITE + HANDLE/RECEIVE/SEND/XCTL/RETURN/SYNCPOINT/ABEND; COPY COACTUP/COCOM01Y/CVCUS01Y/CSUTLDPY/CSLKPCDY resolve; **CSUTLDWY/CSSETATY + DFHAID/DFHBMSCA** stay unresolved holes; `file-io` hole — no invented VSAM/date/REPLACING runtime (prove `online-coactupc-cics-copy-holes`). |
...
| `online/COTRTUPC.cbl` | CardDemo Db2 tran-type **update** program — SQL DML + CICS holes; map `COTRTUP` + INCLUDE DCLTRTYP/DCLTRCAT + CVCRD01Y/CSMSG02Y/CSSTRPFY resolve; CSUTLDWY/CSSETATY unresolved |
| `online/COACTUPC.cbl` | CardDemo account-update **program** — VSAM READ/REWRITE CICS holes; CVCUS01Y/CSUTLDPY/CSLKPCDY resolve; CSUTLDWY/CSSETATY + DFHAID/DFHBMSCA holes |
...
- **Behavioral queue paused (honesty):** gnu-honest subjects are **61/61** (START relation×direction matrix complete). Further Db2/CICS/VSAM/RANDOM/full CSLKPCDY behavioral extracts would invent façades — **refuse** (D6442/D6447). Structural-only CardDemo deepen continues (COACTUPC/COMEN01C/COACTVWC/COSGN00C/**COADM01C**/**COUSR00C**/**COUSR01C–COUSR03C**/**COCRDLIC/COCRDSLC/COCRDUPC**/**COBIL00C**/**COTRN00C–COTRN02C**/**CORPT00C** + batch CB* + **COBSWAIT**/**CSUTLDTC**); keep DFHAID/DFHBMSCA/CSUTLDWY/CSSETATY holes. **CARDONLN LINK targets** with aws-carddemo programs now have dedicated structural fixtures; `COERR00C` is a CARDONLN LINK string only (no CardDemo upstream CBL — refuse inventing an error program). Base `app/cbl` CB*/CO*/CSUTLDTC/COBSWAIT structural floors landed; refuse inventing optional-feature or missing-upstream programs.
...
- Online mini stamps honest `exec-cics` / `copy` holes plus **CICS op catalog** (`execCicsOps`, `handleCondition`, `handleAid`, `respClauses`)
