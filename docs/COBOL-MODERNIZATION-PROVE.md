...
- **Behavioral queue paused (honesty):** gnu-honest subjects are **61/61** (START relation×direction matrix complete). Further Db2/CICS/VSAM/RANDOM/full CSLKPCDY behavioral extracts would invent façades — **refuse** (D6442/D6447).
- **Tier A structural COPY closed (G10075):** Census of `fixtures/hub-cobol-clbs-mini` — **228/271** COPY/INCLUDE refs resolve against `copybook/`; remaining **43** are only **DFHAID** / **DFHBMSCA** / **EXTFMAP** (already-expected BMS holes). **Zero** hole-missing-cpy; no further cheap COPY peels. CardDemo online/batch hole catalogs stay green via `hub:cobol-clbs-prove-smoke`. Next COBOL deepen needs charter (Tier B behavioral >61/61 or Tier C real BMS corpus) — not dialect agents.
