...
- **WebIR deepen (**G10085**–**G10096**):** `buildCobolWebIrHoleAttrs` attaches inventory catalogs to `data.hole` attrs; proven `detectEmitPattern` expecteds lower onto MAIN/sole-entry WebIR literals; arithmetic kinds also emit typed `data.binOp` (`*`/`/`/`+`) while retaining the formatted expected; **G10092** seq-max / perform-varying-sum; **G10093** evaluate-phase / evaluate-func / indexed-key-read; **G10094** indexed/file-io hole attrs; **G10095** nested-if / SEARCH / evaluate-subject / rounded-chain; **G10096** seq-ctl-func-sum / seq-key-* / entry-alt / bill-pipeline + indexed-row family catalogs (no IF/SEARCH/EVALUATE/ISAM invent); `expandCobolCopybooks` expands licensed DFHAID/DFHBMSCA/CMQ* **only when present on disk**; **G10089**/**G10090** inventory + residual (**EXTFMAP** sole open P0). **IBM SDFHCOB:** [`COBOL-IBM-SDFHCOB-DROP.md`](./COBOL-IBM-SDFHCOB-DROP.md). Pattern-emit sidecar remains for cobc∥ multi-lang.
