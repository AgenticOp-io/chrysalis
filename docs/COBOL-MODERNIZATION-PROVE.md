...
- **WebIR deepen (**G10085**–**G10098**):** emit→WebIR catalogs through card-*; **G10098** attaches inventoried HANDLE/RESP/OCCURS/SEARCH/ENTRY/USING to hole attrs, catalogs indexed `meta.limit`, and expands `EXEC SQL INCLUDE` when the book is on disk (SQLCA dual of COPY — no Db2 runtime). **EXTFMAP** sole open P0. **IBM SDFHCOB:** [`COBOL-IBM-SDFHCOB-DROP.md`](./COBOL-IBM-SDFHCOB-DROP.md).
