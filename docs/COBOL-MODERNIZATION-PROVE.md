...
- **WebIR deepen (**G10085**–**G10097**):** `buildCobolWebIrHoleAttrs` attaches inventory catalogs to `data.hole` attrs; proven `detectEmitPattern` expecteds lower onto MAIN/sole-entry WebIR literals; arithmetic kinds also emit typed `data.binOp` (`*`/`/`/`+`) while retaining the formatted expected; **G10092** seq-max / perform-varying-sum; **G10093** evaluate-phase / evaluate-func / indexed-key-read; **G10094** indexed/file-io hole attrs; **G10095** nested-if / SEARCH / evaluate-subject / rounded-chain; **G10096** seq-ctl / seq-key / entry-alt / bill-pipeline + indexed-row family; **G10097** card-pay / card-status / card-account / card-fee-schedule catalogs (no IF/SEARCH/EVALUATE/ISAM invent); CLBS emit→WebIR catalog wave exhausted; `expandCobolCopybooks` expands licensed DFHAID/DFHBMSCA/CMQ* **only when present on disk**; **G10089**/**G10090** inventory + residual (**EXTFMAP** sole open P0). **IBM SDFHCOB:** [`COBOL-IBM-SDFHCOB-DROP.md`](./COBOL-IBM-SDFHCOB-DROP.md). Pattern-emit sidecar remains for cobc∥ multi-lang.
...
- **Tier C AID catalog (**G10084**):** `cicsAidSymbols` / `bmsAttrSymbols` from real upstream (DFHENTER/DFHPF*/DFHBMPRF/…); licensed local DFHAID/DFHBMSCA + CMQ* close residual when present; `EXTFMAP` stays unresolved until ZD&T drop or proven ABSENT.
