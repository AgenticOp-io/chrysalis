...
- **WebIR deepen (**G10085**–**G10090**):** `buildCobolWebIrHoleAttrs` attaches inventory catalogs to `data.hole` attrs; proven `detectEmitPattern` expecteds lower onto MAIN/sole-entry WebIR literals (CLBSMATH / CKPRSTRN / SEQSUM / EMPPAYRN / COPY-linked literals); `expandCobolCopybooks` inlines resolved in-repo `.cpy` (skips DFHAID/DFHBMSCA/EXTFMAP/CMQ*); **G10089** COBOL site-inventory adapter (`--framework cobol`); **G10090** `chrysalis.cobol.residual.v1` ledger (`hub:cobol-residual-ledger`) — P0 proprietary COPY / P1 runtime holes. Pattern-emit sidecar remains for cobc∥ multi-lang.
