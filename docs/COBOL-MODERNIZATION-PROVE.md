...
- **WebIR deepen (**G10085** / **G10086**):** `buildCobolWebIrHoleAttrs` attaches inventory catalogs to `data.hole` attrs (not opaque `handler-body` alone); proven `detectEmitPattern` expecteds lower onto MAIN/sole-entry WebIR literals (e.g. CLBSMATH → `"52.50"`). Pattern-emit sidecar remains for cobc∥ multi-lang; hub path now shares the catalog.
