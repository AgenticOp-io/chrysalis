...
4. **Parallel GTM:** Cursor Pilot Kit 15-min path hardened (`hub:cursor-pilot-kit-smoke` → **`PILOT_KIT_OK`**) + public-claim + **OSS scrub** (`hub:oss-scrub-smoke` → **`OSS_SCRUB_OK`** **G10109**; tracked-tree only — no BFG) — [`CURSOR-PILOT-KIT.md`](./CURSOR-PILOT-KIT.md) · [`PUBLIC-ENGINE-CLAIM.md`](./PUBLIC-ENGINE-CLAIM.md) · [`GO-PUBLIC.md`](./GO-PUBLIC.md)
