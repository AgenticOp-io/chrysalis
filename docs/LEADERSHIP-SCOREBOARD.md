...
**As of:** 2026-07-30 (G10098 hole-attr control surface + indexed limit + SQL INCLUDE expand; G10085–G10097 catalogs; EXTFMAP sole P0)
...
| COBOL primary | **65/65** behavioral; WebIR deepen **G10085–G10098** (emit catalogs + hole-attr control surface + SQL INCLUDE expand; licensed AID+CMQ drop local-only) | no LCB claim; **EXTFMAP** sole P0 COPY + Db2/IMS/MQ runtimes still holes — see [`COBOL-IBM-SDFHCOB-DROP.md`](./COBOL-IBM-SDFHCOB-DROP.md) |
...
| **2c** | COBOL→WebIR deepen (CWL-shaped) | **Closed G10085–G10098** — emit catalogs + hole-attr control/online surface + indexed `limit` + `EXEC SQL INCLUDE` expand-when-present + residual ledger |
...
3. Further inventory peels (CICS FILE/QUEUE literals, BMS crosswalk on holes) — still no runtime invent
