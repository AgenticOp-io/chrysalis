...
- [x] `pnpm run hub:complete-conversion-prove:vue` → `stGreen`+`stClosed` (2026-07-24) — first Vue cwl-api D6448-ST (`hub-flagship-vue` Express API in SFC `<script>` via existing Vue→JS AST lift; express-depth path/query/JSON/status 20/20 hole-free; minimal template shell only; no invented product UI)  
- [x] `pnpm run hub:complete-conversion-prove:cpp` → `stGreen`+`stClosed` (2026-07-24) — first C++ cwl-api D6448-ST (`hub-flagship-cpp` silver file-lift GET-literal 20/20 hole-free); **not** express-depth — blocker: no C++ HTTP framework AST for verbs/JSON/params/status  
