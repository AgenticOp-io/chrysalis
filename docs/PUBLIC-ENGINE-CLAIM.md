...
- [x] `pnpm run hub:complete-conversion-prove:rust` → `stGreen`+`stClosed` (2026-07-24) — first Rust cwl-api D6448-ST (`hub-flagship-rust` Actix Web 20/20 hole-free; scalar/`serde_json::json!`/`HttpResponse` status + path/query; no invented UI); secondary Axum dialect hole-free (`pnpm run hub:axum-smoke` / `hub-gold-axum` 20/20)  
...
- [x] `pnpm run hub:axum-smoke` → Axum Rust origin dialect hole-free (`hub-gold-axum` 20/20; secondary to Actix Rust ST)  
