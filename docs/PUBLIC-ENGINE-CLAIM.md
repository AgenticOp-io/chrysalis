...
- [x] `pnpm run hub:restify-smoke` → Restify TypeScript origin dialect hole-free (`hub-gold-restify` 20/20 `server.get|post|…|del` + `res.send` / Restify `res.send(code, body)` + `req.params|query`; secondary to Express/TS ST; plugins/pre unwired)  
- [x] `pnpm run hub:polka-smoke` → Polka TypeScript origin dialect hole-free (`hub-gold-polka` 20/20 `app.get|post` + `res.writeHead` / `res.end(JSON.stringify)` + `req.params|query`; secondary to Express/TS ST; completes thin Node set)  
