...
- [x] `pnpm run hub:dart-smoke` → Dart/Shelf foundation hole-free (`hub-gold-dart-shelf` 20/20 `router.get|post` + `Response.ok`/`Response(status)` + `jsonEncode` + query/body + `<id>`; Flutter/Dart Frog/Pipeline unwired; not ST)  
