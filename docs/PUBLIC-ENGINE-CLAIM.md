...
- [x] `pnpm run hub:complete-conversion-prove:kotlin` → `stGreen`+`stClosed` (2026-07-24) — first Kotlin cwl-api D6448-ST (`hub-flagship-kotlin` Spring `@RestController` 20/20 hole-free; brace/expression `fun` + mapOf/ResponseEntity + path/query; no invented UI); secondary Ktor dialect hole-free (`pnpm run hub:ktor-smoke` / `hub-gold-ktor` 20/20 `routing { get|post|… }` + `call.parameters` + `queryParameters` + `HttpStatusCode`)  
