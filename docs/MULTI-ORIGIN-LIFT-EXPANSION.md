...
| **G10072** | Fastify encapsulated plugin prefix peel | `register(fn, { prefix: '/api' })` + literal `app.get|post` path join (no plugin runtime invent) | `hub:fastify-prefix-smoke` ✅ |
...
| **G10068** | Actix Web scope nest path join | Literal `web::scope("/prefix").service(handler)` / `.route` + relative macros; flagship stays flat ST; no guard invent | `hub:actix-scope-smoke` ✅ |
...
| **G10007** | Dart Shelf same-file named handlers | `router.get('/x', myHandler)` → same-file `Response\|Future<Response> myHandler(Request …)` (Axum/Go parallel); cross-file named = honest hole | `hub:dart-smoke` ✅ |
...
| **G10044** | Hono pass-through middleware | Hono ORIGIN `app.use` empty/next-only → `js.passthrough` (G9959 parallel; **≠ emit-hono**); complex mw = honest holes | `hub:hono-smoke` v2 ✅ |
...
| **G10038** | Go Iris secondary dialect | `iris.New` + `app.Get\|Post\|…` + `{id}`/`:id` + `ctx.Params().Get` / `ctx.URLParam`/`URLParamDefault` + `ctx.JSON`/`ctx.WriteString` (secondary; Gin remains Go ST) | `hub:iris-smoke` ✅ |
...
| **G10019** | Hono TS ORIGIN secondary dialect | `new Hono()` + `app.get|post|…` + `c.req.param`/`c.req.query` + `c.json`/`c.text` (secondary; Express/TS remain ST; **≠ emit-hono**; pass-through mw G10044) | `hub:hono-smoke` ✅ |
| **G10047** | itty-router JS/TS ORIGIN secondary dialect | `Router()` + `router.get|post|…` + `:id` + `request.params` + URL `searchParams` + `json()`/`Response.json`/`new Response` (secondary; Express/TS remain ST) | `hub:itty-smoke` ✅ |
| **G10063** | Cloudflare Workers fetch-export ORIGIN secondary | `export default { async fetch }` + method/pathname switch + `Response.json`/`{ status }` + `url.searchParams` (secondary; Express/TS remain ST; itty remains Workers router) | `hub:cf-workers-smoke` ✅ |
| **G10069** | Vapor route group peel | `app.grouped("prefix")` + nested/chained literal PathComponent join with `.get\|.post\|…` (deepen; Vapor remains Swift ST; Hummingbird must not regress; Fluent/Leaf stay holes) | `hub:vapor-group-smoke` ✅ |
| **G10059** | AdonisJS TS ORIGIN secondary dialect | `Route.get|post|…` / `router.get` + `:id` + `request.param` / `request.qs()` + `response.json`/`response.status` (secondary; Express/TS remain ST; no Lucid/IoC) | `hub:adonis-smoke` ✅ |
...
| **G10035** | Javalin Java secondary dialect | `Javalin.create` + `app.get\|post\|…` + `{id}` + `ctx.pathParam`/`ctx.queryParam` + `ctx.status(n).json`/`ctx.json`/`ctx.result` (secondary; Spring remains Java ST; plugins/DI = honest holes) | `hub:javalin-smoke` ✅ |
| **G10036** | Spark Java secondary dialect | `spark.Spark.get\|post\|…` + `:id` paths + `req.params`/`req.queryParams` + `res.status`/`res.type` + string/JSON (secondary; Spring remains Java ST; filters/static/WebSocket = honest holes) | `hub:sparkjava-smoke` ✅ |
| **G10046** | Jooby Java secondary dialect | `new Jooby() {{ get\|post\|… }}` / `app.get\|post\|…` + `{id}` + `ctx.path`/`ctx.query` + `ctx.setResponseCode` + Map/string (secondary; Spring remains Java ST; module/MVC/filters = honest holes) | `hub:jooby-smoke` ✅ |
| **G10042** | Helidon MP JAX-RS Java secondary dialect | `jakarta.ws.rs` `@Path`+`@GET\|POST\|…` resource surface **via G10012 peels** (secondary; Spring remains Java ST; CDI/MP Config/Helidon SE = honest holes) | `hub:helidon-smoke` ✅ |
