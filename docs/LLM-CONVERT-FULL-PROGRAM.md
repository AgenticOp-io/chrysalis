...
> **Status:** **Program closed** (2026-07-03, **G8940**) — was **active** (**G8900**, **D6303** — user-amended)  
...
| **43a.3** | Apply → `@chrysalis/repair` hole-closure bridge | **G8913** (`hub:llm-convert-repair-bridge-smoke`) |
| **43b** | MCP tools: enrich, verify gate, apply | **G8921** (composed in build slice) |
| **43c** | Program close + Phase 42 regression | **G8940** (`hub:llm-convert-full-close-smoke`) |
...
- Trajectory logging for enrich, verify gate, apply, repair bridge
- `@chrysalis/repair` hole-closure when traces + `CHRYSALIS_HUB_VERIFY_BASE_URL` available
...
| `hub_convert_apply_holes` | Apply after verify + `confirmApply: true`; invokes repair bridge when patch is hole-closure |
