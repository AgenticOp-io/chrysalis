...
- **GCE smoke:** `scripts/gce-wptp-test-vm.ps1` + `gce-wptp-test-bootstrap.sh` (e2-small; [HOW-TO §25](./HOW-TO.md#25-smoke-test-wptp-matrix-on-gce)). **2026-05-26:** `chrysalis-test-vm` @ **`wptp-matrix@v0.1.10`** — validate **24 edges**, vitest + **`verify:harness` OK** (~7m after fresh `npm ci`). Silver Chrysalis cases skip without **`CHRYSALIS_ROOT`** on the VM (matrix-only smoke). Bootstrap reclones when **`MATRIX_REF`** changes (`.chrysalis-matrix-ref` stamp).
