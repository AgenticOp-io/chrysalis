...
**Shared VM (Chrysalis + other projects).** On a host such as **`chrysalis-test-vm`**, another project may own **nginx :80** and its own status API (e.g. **fragility** on **`127.0.0.1:8765`**). Chrysalis bootstrap only replaces **`~/chrysalis-test`** (and uploads **`~/chrysalis-src.tgz`** / **`~/gce-test-vm-bootstrap.sh`**). It does **not** modify **`~/fragility-discovery-engine`**, FDE nginx sites (**`fragility-default-ip`**, **`fragility-public`**), or port **8765**. **Public hub edge (locked D6396):** site **`chrysalis-hub`** only — [`HUB-DEMO-INSTALL.md`](./HUB-DEMO-INSTALL.md), [`nginx/chrysalis-hub.vhost.example`](./nginx/chrysalis-hub.vhost.example), `pnpm run deploy:hub-caddy-tls`. **Automated hub deploy:** **`pnpm run deploy:hub-demo`** or **`scripts/gce-hub-deploy.ps1 -Project <id>`** — local **`build:hub-all`**, tarball refresh, **`gce-hub-finish-deploy.sh`** (verify + restart hub). Refresh-only: **`scripts/gce-test-vm-refresh.ps1`** (**`CHRYSALIS_REFRESH_ONLY=1`**; installs **php-cli** on refresh if missing). Bootstrap runs **`pnpm -r build`**, parser vendor, **`install-wptp-hub-deps.mjs`**, **`hub-post-deploy-verify.mjs`**. Env: **`CHRYSALIS_SKIP_WPTP_HUB_DEPS=1`**, **`CHRYSALIS_SKIP_HUB_FINISH=1`**, **`CHRYSALIS_DEPLOY_STRICT=0`**. Local build: **`pnpm run build:hub-all`**. Hub UI: **`chrysalis-operator-web.mjs`** on **:19090** — **`docs/HUB-DEMO-INSTALL.md`**, **`/#/guide`**, **`/docs/hub-install`**. Front page + **new project** wizard (all languages by default; optional SSH autodetect via **`chrysalis-origin-scan`** on origins), target selection, optional **`scp`** pull into **`~/.chrysalis-hub/workspaces/`**, then **console** with live ingest (SSE). All servers and stores are reached by **SSH** or **local install** on hub/origin — no Chrysalis cloud agents. See **[Hub connectivity](./HUB-CONNECTIVITY.md)** (origin agent install, DB/Redis via local path or SSH tunnel, **`chrysalis-hub-probe.mjs`**). Run the hub on a VM that has **SSH key access** to every origin server you need to recode. Prefer hub bind **`127.0.0.1:19090`** behind **`chrysalis-hub`** nginx; public **https://hub.agenticop.io**. Optional **`CHRYSALIS_OPERATOR_TOKEN`** for POST auth.
