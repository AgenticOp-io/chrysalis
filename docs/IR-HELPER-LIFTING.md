...
**Status:** **B1–B5.5 v9** on `main` (fixtures + Vitest + simulate + oracle/replay gates + parametric lib-helper inlining + literal-RHS assign + **`__cast_int`** formal assign + emit **`lib-helpers.ts`** on Hono/Fastify + **`/zeta`** replay twin). Ingest normalizes SQL whitespace, keyword case (quote-aware), and inlines lib query helpers at call sites. Hub: **semantic smoke v5** (`sql-param-inline`) and **emit replay twin gate** (sql-same + sql-case + param-inline twins) in completion batch (**G2303–G2304**).  
