...
| **B5.4** | SQL keyword case normalization for effectful semantic lift keys. | **Done (v1–v4)** — keyword uppercasing after whitespace collapse; **v2:** oracle twin verify; **v3:** quote-aware literal handling in **`normalizeSqlLiteralForHelperLift`**; **v4:** Vitest edge cases (double-quoted literals, escaped quotes). |
| **B5.5** | Ingest inlining of lib helpers whose body is **`return <effect.db.query>`** (zero-arg, parametric, or assign-then-return). | **Done (v1–v5)** — **`tryInlineLibHelperCall`** + **`HelperBodyEntry`**; **v2:** emit replay for sql-same-twin; **v3:** parametric + local-assign bodies + sql-case-twin replay + fixture **`lift-helper-sql-param-inline`**; **v4:** emit replay for sql-param-inline; **v5:** multi-assign local chains (**`sql_param_chain`**). |
