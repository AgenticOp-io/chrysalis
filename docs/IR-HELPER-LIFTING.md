...
| **B5.5** | Ingest inlining of lib helpers whose body is **`return <effect.db.query>`** (zero-arg, parametric, or assign-then-return). | **Done (v1–v8)** — **`tryInlineLibHelperCall`** + **`HelperBodyEntry`**; replay twins; parametric/chain bodies; **v5.1:** negative control (**`sql_param_noinline`**); **v6:** skip effect-free prelude expr stmts (**`sql_param_prelude`**); **v7 (G2318–G2319):** emit **`lib-helpers.ts`** for non-inlinable helpers + effectful-prelude control (**`sql_param_sideeffect`**); **v8 (G2325–G2326):** literal-RHS assign inlining + **`/zeta`** emit replay twin. |
