...
| **B5.5** | Ingest inlining of lib helpers whose body is **`return <effect.db.query>`** (zero-arg, parametric, or assign-then-return). | **Done (v1–v6)** — **`tryInlineLibHelperCall`** + **`HelperBodyEntry`**; replay twins; parametric/chain bodies; **v5.1:** negative control (**`sql_param_noinline`**, ingest-only); **v6:** skip effect-free prelude expr stmts (**`sql_param_prelude`**). |
