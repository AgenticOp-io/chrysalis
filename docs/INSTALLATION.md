...
| `git: 'credential-manager-core' is not a git command` (Windows) | **`~/.gitconfig`** sets **`credential.helper=manager-core`**, which is not a valid helper name on your PATH, while Git’s **system** config already sets **`credential.helper=manager`** (Git Credential Manager). | Run **`git config --global --unset credential.helper`** so the system **`manager`** helper is used; or set **`git config --global credential.helper manager`** explicitly. Reinstall [Git for Windows](https://git-scm.com/download/win) if helpers are missing entirely. |
