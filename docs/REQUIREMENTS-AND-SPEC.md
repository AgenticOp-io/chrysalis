...
- Parse PHP sources into a canonical AST representation consumable from Node.
...
- **CLI** orchestration, **status** aggregation, optional **repair** loop gated by verify, **archaeology** (schema + traces → typed domain artifacts), and **CI-facing JSON artifacts** where documented.
...
- Ambient nondeterminism in generated handlers or verify sandhooks where replay fidelity is required (`DESIGN.md` principle 7; injected context instead).
...
| **Oracle** | Subsystem that records traces from the legacy stack (PHP prelude + Node-side corpus I/O). |
...
| FR-002 | P0 | The system SHALL support **replay** of a corpus against a **running** emitted base URL with deterministic ordering policy documented for the verifier. | `packages/verify/README.md` |
| FR-003 | P1 | Verify SHALL produce a **per-route** and **aggregate** correctness report and persist artifacts under a configurable report directory. | `DESIGN.md` §5.2 |
...
| ID | Priority | Requirement | Design trace |
|----|----------|-------------|--------------|
| FR-010 | P0 | The system SHALL parse PHP source into a **versioned, canonical AST JSON** shape (`PhpAst`) without executing PHP in the parser bridge. | `packages/parser-bridge/README.md` |
...
| FR-015 | P2 | The system MAY support **route-level ingest sharding** and **merging** of WebIR modules with documented dedupe semantics. | `DESIGN.md` D246–D247; `packages/ingest/README.md` |
...
| FR-022 | P1 | The system SHALL record dimensions needed for replay fidelity where supported: HTTP request/response, SQL text and optional rows, session mutations, time/RNG observations as documented. | `DESIGN.md` §5.2 |
...
### 3.5 Provenance, effects, and determinism

| ID | Priority | Requirement | Design trace |
|----|----------|-------------|--------------|
| FR-040 | P0 | Generated artifacts that represent domain or API shapes SHALL carry **provenance** sufficient for audit (e.g. JSDoc `@chrysalis-provenance` where that pattern is used). | `DESIGN.md` §3.3, §6.3 |
...
| FR-042 | P1 | Generated code and verify replay paths SHALL use **injectable** time and randomness (no `Date.now()` / `Math.random()` in generated handlers where determinism is required). | `DESIGN.md` §3.7; `AGENTS.md` §2 |
...
| FR-052 | P2 | Documented **CI gate scripts** SHALL validate versioned JSON artifacts (`kind`, `schemaVersion`) without silent failure modes inconsistent with `DESIGN` D231. | Root `README.md` ci-gates; `AGENTS.md` |

### 3.7 Licensing (optional product gate)

| ID | Priority | Requirement | Design trace |
|----|----------|-------------|--------------|
| FR-060 | P2 | When enabled via environment variables, the CLI SHALL enforce **commercial license** checks per `docs/COMMERCIAL.md` without changing IR or oracle semantics. | `DESIGN.md` D289 |
...
| `@chrysalis/emit-shared` | Shared emit strategies and artifacts. |
| `@chrysalis/oracle` | Corpus model, read/merge, observe session API (Node). |
| `packages/oracle-php` | PHP prelude: capture, redaction, session bridge hooks. |
...
### 4.4 Trace and verify artifacts (interfaces)
...
- **Verify report:** `CorrectnessReport` + on-disk `summary.json` and per-route files; field semantics in `@chrysalis/verify/README.md`.
...
| NFR-004 | CI | Root `pnpm test` and documented `pnpm run ci:*` gates enforce artifact schemas where claimed (`AGENTS.md` local ci-gates). |
