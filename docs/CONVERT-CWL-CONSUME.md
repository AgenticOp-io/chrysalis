...
**Status:** Convert consumer aligned to CWL **`1.0.39`** — **Rosetta Step 2 (Translation) closed**  
**Date:** 2026-09-14
...
| Language pin | `file:../chrysalis-cwl/packages/cwl` ≡ **`1.0.39`** (`hub:cwl-pin-smoke`) |
...
**Registry:** `@agenticop-io/cwl@1.0.39` via [`.npmrc.example`](../.npmrc.example) (do not commit tokens).

```json
"@agenticop-io/cwl": "1.0.39"
```
...
```text
CONVERT_GRAVITY: ok
CWL_PIN: file:1.0.39 (registry @agenticop-io/cwl@1.0.39 ready via .npmrc.example)
