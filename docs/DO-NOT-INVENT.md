...
**G10085–G10097:** WebIR deepen — shaped holes, COPY expand (licensed DFHAID/DFHBMSCA/CMQ* expand when on disk; else skip), emit-pattern literals + typed binOp / catalogs through card-* / seq-ctl / indexed-row (CLBS catalog wave exhausted). Still refuse inventing EXTFMAP/DFHATTR stubs or Db2/IMS/MQ runtimes. Operator SDFHCOB path: [`COBOL-IBM-SDFHCOB-DROP.md`](./COBOL-IBM-SDFHCOB-DROP.md) (books **gitignored** — never publish).
