...
| **B — IR helper lifting** | **Origin-insensitive** optional **D283** dedupe (**different PHP origins**, same structure). **Future:** lift shared helpers when bodies are **not** structurally identical even ignoring origin. Design pass: **`docs/IR-HELPER-LIFTING.md`** (**D311**). | **`merge-dedupe-key.ts`**, **`dedupeStructuralSubgraphsInModule({ ignoreOrigin })`**, **`IngestOptions.dedupeStructuralSubgraphsIgnoreOrigin`**, CLI **`--ingest-dedupe-structural-subgraphs-ignore-origin`**. | [#3](https://github.com/theorem6/chrysalis/issues/3) |
