...
| **B — IR helper lifting** | **B1** fixtures + **B2 v0** canonicalize structurally identical lib helper bodies for call-effect widening (**`liftSharedHelpers`**, CLI **`--ingest-lift-shared-helpers`**, requires structural dedupe). **Future (B3):** semantic merge when locals differ (**`lift-helper-gap-probe`**). Design: **`docs/IR-HELPER-LIFTING.md`** (**D311**, **D323**). | **`lift-shared-helpers.ts`**, **`merge-dedupe-key.ts`**, **`dedupeStructuralSubgraphsInModule`**, fixtures **`lift-helper-*`**. | [#3](https://github.com/theorem6/chrysalis/issues/3) |
