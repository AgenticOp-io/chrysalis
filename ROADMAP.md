...
## Status (2026-07)
- **Closed (2026-08-05):** **cwl-fmt dual-mode locked (D6548)** — pillar = parse→print; convert keeps WebIR `cwl-fmt` (`fmt:cwl:webir`). Doc: [`docs/CWL-FMT-DUAL-MODE.md`](./docs/CWL-FMT-DUAL-MODE.md). DNA enforce / cutover identity compare → Secure; Convert does not own firewall. Prove: `hub:cwl-language-pillar-smoke`.
- **Closed (2026-07-27):** **Tier A+ queue hygiene (G10077 / D6538)** — default build queue is **COBOL primary** (`docs/LEADERSHIP-SCOREBOARD.md`, `docs/COBOL-PRIMARY-UNIVERSAL-BUILD.md`, strategic §12); secondary-dialect/prefix bingo **paused** unless explicitly asked. Tier A COPY (G10075) + Tier B Small `ckprstdn` (G10076) already closed; further Db2/CICS/VSAM/BMS need charter. No LCB claim.
- **Closed (2026-07-27):** **Fastify encapsulated plugin prefix peel** — `hub:fastify-prefix-smoke` / `hub-gold-fastify-prefix` 20/20 hole-free (`register(fn, { prefix: '/api' })` + literal `app.get|post` path join); base `hub:fastify-smoke` unchanged; no plugin runtime invent (G10072 / D6534). Catalog: `fixtures/ci/js-secondary-dialect-honest-holes.json`.
- **Closed (2026-07-27):** **Spring MVC class+method `@RequestMapping` peel deepen** � `hub:spring-requestmapping-smoke` / `hub-gold-spring-requestmapping` 20/20 hole-free (class `@RequestMapping` prefix join + method `@*Mapping` + method-level `@RequestMapping(method=RequestMethod.*)` + multi-path arrays); Spring `hub-flagship-java` remains Java D6448-ST; Micronaut/JAX-RS/WebFlux stay green (G10071 / D6533). DI/filters / method `@RequestMapping` without `RequestMethod.*` = honest holes. Catalog: `fixtures/ci/java-secondary-dialect-honest-holes.json`.

- **Releases:** `v1.0.0` -> `v2.0.x` tagged on `main`.
- **Active lane:** **COBOL primary** (Tier A/A+/B-Small closed; Medium+/BMS charter) + **Post?G9990 maintain** ? **translate-only** (**D6442**) + **source-authoritative UI** (**D6443**) + **origin source corpus / piecemeal queue** (**D6444** / **G9993**); Migration OS (**G8550**); UT regression (**G7690**). WISP = POC only; GenieACS OOS; **LiteRT.js refused**. **Maps = ArcGIS when source is ArcGIS**.
- **Closed (2026-07-27):** **Sinatra namespace/prefix path peel** ? `hub:sinatra-ns-smoke` / `hub-gold-sinatra-ns` 20/20 hole-free (`namespace "/api" do` + flat `get|post|?` path join; deepens Sinatra D6448-ST); Roda/Grape/Padrino stay green; Rack map / conditions / invented `base.path` = honest holes (G10073 / D6535). Catalog: `fixtures/ci/sinatra-ns-honest-holes.json`. Rails stays skipped (G10006).
- **Closed (2026-07-27):** **Gin route Group prefix peel** ? `hub:gin-group-smoke` / `hub-gold-gin-group` 20/20 hole-free (literal `r.Group("/prefix")` + nested/chained `g.GET|POST` path join); flat Gin ST remains `hub-flagship-go` / `hub:go-flagship`; Chi/Echo stay green; non-literal Group / Group middleware args / `g.Use` = honest holes (G10066 / D6528). Catalog: `fixtures/ci/go-secondary-dialect-honest-holes.json`.
- **Closed (2026-07-27):** **Vapor route group peel** ? `hub:vapor-group-smoke` / `hub-gold-vapor-group` 20/20 hole-free (`app.grouped("prefix")` + nested/chained literal PathComponent join with `.get|.post|?`); Vapor remains Swift D6448-ST; Hummingbird stays green; Fluent/Leaf/middleware-grouped = honest holes (G10069 / D6531). Catalog: `fixtures/ci/swift-vapor-group-honest-holes.json`.
- **Closed (2026-07-27):** **Express Router mount peel** ? `hub:express-router-smoke` / `hub-gold-express-router` 20/20 hole-free (`express.Router()` + `router.get|post` + `app.use('/api', router)` literal path join; empty pass-through OK per G9959; complex `use(prefix, mw, router)` = honest hole). Express/TS remain D6448-ST (G10067 / D6529). Catalog: `fixtures/ci/js-secondary-dialect-honest-holes.json`.
- **Closed (2026-07-27):** **Flask Blueprint secondary peel** ? `hub:flask-blueprint-smoke` / `hub-gold-flask-blueprint` 20/20 hole-free (same-file `Blueprint('name', ?, url_prefix=?)` + `@bp.get|post|route` + literal prefix join); Flask `@app.*` remains Python D6448-ST; FastAPI/Starlette/?/Tornado stay green; cross-file Blueprint / register override / nested / middleware = honest holes (G10070 / D6532). Catalog: `fixtures/ci/flask-blueprint-honest-holes.json`.
- **Closed (2026-07-27):** **Revel Go secondary skipped** ? not cheap on go-ast-ingest (`conf/routes` + `Controller.Action` / `revel.Result` is Rails-class cross-file resolve like G10006; no Gin/Buffalo/Martini peel-reuse; probe real routes lifted 0; Params.*.Get false-positive `.Get` matches only). Gin ST + Chi/Echo/Fiber/Iris/Beego/Buffalo/Martini/Gorilla/ServeMux stay green. Catalog: `fixtures/ci/revel-honest-skip.json` (G10065). No D6527.
- **Closed (2026-07-27):** **Actix Web scope nest path-join deepen** ? `hub:actix-scope-smoke` / `hub-gold-actix-scope` 20/20 hole-free (`web::scope("/prefix").service(handler)` / `.route` + relative `#[get|post|?]` path join); flagship `hub-flagship-rust` stays flat D6448-ST; Axum/Rocket/Poem/Salvo stay green (G10068 / D6530). Nested scope / `.guard` / `web::resource` = honest holes. Catalog: `fixtures/ci/actix-scope-honest-holes.json`.
- **Closed (2026-07-27):** **Cloudflare Workers fetch-export JS/TS ORIGIN secondary dialect** ? `hub:cf-workers-smoke` / `hub-gold-cf-workers` 20/20 hole-free (`export default { async fetch }` + switch/if on `request.method` + `url.pathname` + `Response.json`/`{ status }` + `url.searchParams`); Express/TS remain D6448-ST; itty remains Workers router dialect; KV/D1/env/dynamic segments = honest holes (G10063 / D6525). Catalog: `fixtures/ci/js-secondary-dialect-honest-holes.json`.
- **Closed (2026-07-27):** **AdonisJS TypeScript ORIGIN secondary dialect** ? `hub:adonis-smoke` / `hub-gold-adonis` 20/20 hole-free (`Route.get|post|?` / `router.get` in `start/routes.ts` + `:id` + `request.param` / `request.qs()` + `response.json`/`response.status`); Express/TS remain D6448-ST; Lucid/IoC/controller string refs = honest holes (G10059 / D6521). Catalog: `fixtures/ci/js-secondary-dialect-honest-holes.json`.
- **Closed (2026-07-27):** **Spring WebFlux RouterFunctions Java secondary dialect** ? `hub:webflux-smoke` / `hub-gold-webflux` 20/20 hole-free (`route(GET|POST|?("/path"), req -> ?)` / `.andRoute` + `{id}` + `pathVariable`/`queryParam` + `ServerResponse.ok|status().bodyValue`); Spring MVC `@RestController` remains Java D6448-ST; JAX-RS/Javalin/Jooby/Vert.x peers stay green (G10061 / D6523). WebClient/route-builder/`path` nest/filters = honest holes. Catalog: `fixtures/ci/java-secondary-dialect-honest-holes.json`.
- **Closed (2026-07-27):** **Padrino Ruby secondary dialect** ? `hub:padrino-smoke` / `hub-gold-padrino` 20/20 hole-free (`Padrino.configure_apps` + `class HubApp < Padrino::Application` + flat `get|post|? "/path"` + `/:id` + Hash/`status`/`params[]`; reuses Sinatra peels like Grape); Sinatra remains Ruby D6448-ST; Roda/Grape stay green (G10062 / D6524). Symbol controllers/`Padrino.mount`/filters = honest holes. Catalog: `fixtures/ci/padrino-honest-holes.json`. Rails stays skipped (G10006).
- **Closed (2026-07-27):** **Tapir Scala secondary skipped** ? not cheap on scala-ast-ingest (fluent `endpoint.get.in(...).out(codec...)` / serverLogic is not Finch/Http4s Ok peel-reuse; codec invent forbidden; probe 0 routes). Akka ST + Http4s/Finch stay green. Catalog: `fixtures/ci/tapir-honest-skip.json` (G10057). No D6519.
- **Closed (2026-07-27):** **Martini Go secondary dialect** ? `hub:martini-smoke` / `hub-gold-martini` 20/20 hole-free (`martini.Classic` + `m.Get|Post|?` + `:id` + `params["id"]` + `r.JSON` / `json.NewEncoder(w)`); Gin remains Go D6448-ST (G10056 / D6518). Use/Group/Map/binding = honest holes. Catalog: `fixtures/ci/go-secondary-dialect-honest-holes.json`.
...
- **Closed (2026-07-27):** **G10074 HAR/OpenAPI query-param deepen** ? OpenAPI in: query schema.default else example ? CWL query; HAR IDENT-safe queryString ? CWL query with observed value when present; gold fixtures + hub:contract-import-cwl-roundtrip-smoke + Vitest G10074 (D6536). Absent / hyphenated / schema-only stay name-only or unwired (no invent).
...
- **Closed (2026-07-27):** **itty-router empty/next-only pass-through middleware peel** ? `hub:itty-smoke` v2 / `hub-gold-itty` 20 routes + 2 `js.passthrough` empty `router.all('*', () => {})` presets; itty has **no** onion `next` (no-return continues); complex `all` / nested Router = honest holes (G10064 / D6526; parallel G10044 / G9959 / G10053). Catalog: `fixtures/ci/js-secondary-dialect-honest-holes.json`.
...
- **Closed (2026-07-27):** **Deno.serve TypeScript ORIGIN secondary skipped** ? not cheap on javascript-ast-ingest (no Bun-like `Deno.serve({ routes })`; pathname/method switch + URLPattern / `@std/http` route = control-flow peels; D6510 already holes switch-only servers; probe `astRouteCount` 0). Oak / Bun.serve / itty stay green. Catalog: `fixtures/ci/deno-serve-honest-skip.json` (G10060 / D6522).
...
- **Closed (2026-07-27):** **Go Buffalo secondary dialect** ? `hub:buffalo-smoke` / `hub-gold-buffalo` 20/20 hole-free (`app.GET|POST|?` + `{id}` + `c.Param` + `c.Render`/`r.JSON`/`r.String`); Gin remains Go D6448-ST (G10055 / D6517). Middleware/Group/Resource/binders = honest holes. Catalog: `fixtures/ci/go-secondary-dialect-honest-holes.json`.
...
- **Closed (2026-08-07):** **Phoenix controller route-surface** ? `hub:phoenix-controllers-smoke` / `hub-gold-phoenix-controllers` 20/20 hole-free (`Ctrl, :action` + `json`/`put_status`; G10126 / D6540). LiveView remains honest hole (**D6447**). Plug.Router remains Elixir ST.
