...
| **C — Verify UX** | Operators can **act** on failure: which trace, which route, which divergence class, what to run next. | `replayCorpus`, report JSON, CLI | **D212:** histogram + absolute `summary.json` + repair / `--project` hints on **`chrysalis verify`**. Next: narrow replay (`--route` / trace filter), richer stderr, replay env doc consolidation. |
