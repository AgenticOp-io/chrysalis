...
| **B — IR helper lifting** | **B1–B3 v0:** fixtures + **`liftSharedHelpers`** / **`liftSharedHelpersSemantic`** (CLI **`--ingest-lift-shared-helpers`**, **`--ingest-lift-shared-helpers-semantic`**; requires structural dedupe). Merges identical lib bodies and locals-only renames for call-effect widening. Design: **`docs/IR-HELPER-LIFTING.md`** (**D311**, **D323**, **D324**). | **`lift-shared-helpers.ts`**, **`merge-dedupe-key.ts`**, fixtures **`lift-helper-*`**. | [#3](https://github.com/theorem6/chrysalis/issues/3) |
