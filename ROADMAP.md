...
**Status: in progress (D84–D107: canonical worktree, Breeze coexistence, template oracle growth).** This milestone **does not reopen** the M4 v1
...
- [ ] Larger oracle corpora than scripted drivers; pipeline-owned **idiomaticity** and
      **residual-legacy** JSON when those numbers should gate releases. **Incremental (D86–D107):**
      **`GET /chrysalis-min-id`** / **`GET /chrysalis-max-id`** / **`GET /chrysalis-avg-id`** / **`GET /chrysalis-id-span`**
      ...
      + `WHERE`-filter counts x2 each) on **`chrysalis-templates/`** — **thirty-nine**
