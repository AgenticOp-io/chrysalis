...
| **B — IR helper lifting** | **Origin-insensitive** optional **D283** dedupe (**different PHP origins**, same structure). **Future:** lift shared helpers when bodies are **not** structurally identical even ignoring origin. | **`merge-dedupe-key.ts`**, **`dedupeStructuralSubgraphsInModule({ ignoreOrigin })`**, **`IngestOptions.dedupeStructuralSubgraphsIgnoreOrigin`**, CLI **`--ingest-dedupe-structural-subgraphs-ignore-origin`**. | [#3](https://github.com/theorem6/chrysalis/issues/3) |
...
**Wave 6 (2026-05-20):** **Lane A** — **`fixtures/parser-parity-probe`** **`coalesce_assign.php`** (`??=`) + **`string_interpolation.php`** with nikic/glayzzle parity tests; **D7** operator helper **`pnpm run wptp:d7-audit`**; flagship empty-seed + Hono **`__respond`** + CLI stress-report status fixes (see **`CHANGELOG.md`** [Unreleased]).
