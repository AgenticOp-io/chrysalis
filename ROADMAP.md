...
- [x] **Ingest progress JSON (diagnostic, v0):** **`chrysalis.ingest.progress`** **`schemaVersion` 0** written when **`--ingest-progress-file <path>`** is set on **`ingest` / `emit` / `status --project`**; **`IngestOptions.ingestProgressFile`** in **`@chrysalis/ingest`**. Vitest **`ingest-progress-file.test.ts`** + CLI **`ingest-progress-file-cli.test.ts`**. **DESIGN D277** (forensics only; incompatible with **`--merge-all-shards`**).
