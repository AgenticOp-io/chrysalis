...
- **Phase 8 product proof:** **`pnpm run hub:strategic-plan-phase8-product-proof-close-smoke`** with **`CHRYSALIS_STRICT_STRATEGIC_PLAN=1`** (or on **`GITHUB_ACTIONS`**) runs oracle/HTTP/CWL/Hub pillars without **`CHRYSALIS_STRATEGIC_PLAN_SKIP_*`**; GCE: **`scripts/gce-strategic-plan-phase8-strict.sh`** (**D6110**). Vitest **`hub-strategic-plan-phase8.test.ts`** uses explicit skips locally.
...
- **Default long test runs:** **`pnpm run test:gce`** uploads local **`git HEAD`**, syncs runner scripts, and starts **`scripts/gce-run-all-tests.sh`** detached on **`chrysalis-test-vm`** (see **`docs/GCE-LOCAL-VERIFY.md`**). **`pnpm run test:gce:status`** / **`test:gce:fetch`**. Optional full workspace Vitest: **`.\scripts\gce-run-all-tests.ps1 -FullVitest -Detach`**.
