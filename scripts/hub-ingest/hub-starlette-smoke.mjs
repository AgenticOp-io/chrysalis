#!/usr/bin/env node
/**
 * Smoke: hub-gold-starlette Starlette dialect → WebIR hole-free (20 routes).
 * Does not replace Flask hub-flagship-python D6448-ST.
 */
...
export const HUB_STARLETTE_SMOKE_KIND = "chrysalis.hub.starlette-smoke";
export const HUB_STARLETTE_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-starlette");
 ...
 */
export async function runStarletteSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "app.py");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_STARLETTE_SMOKE_KIND,
      schemaVersion: HUB_STARLETTE_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-py",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runStarletteSmoke();
