#!/usr/bin/env node
/**
 * Smoke: hub-gold-vertx Vert.x Web dialect → WebIR hole-free (20 routes).
 * Does not replace hub-flagship-java Spring D6448-ST.
 * No EventBus invent (D6447). G10052 / D6514.
 */
...
export const HUB_VERTX_SMOKE_KIND = "chrysalis.hub.vertx-smoke";
export const HUB_VERTX_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-vertx");
 ...
 */
export async function runVertxSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "HubApp.java");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_VERTX_SMOKE_KIND,
      schemaVersion: HUB_VERTX_SMOKE_SCHEMA_VERSION,
      ok: false,
  ...
  return {
    ...
    routeCount,
    holeCount,
    vertxRouteCount: liftReport.astRouteCount ?? null,
...
async function main() {
  const report = await runVertxSmoke();
