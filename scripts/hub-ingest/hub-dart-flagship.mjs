#!/usr/bin/env node
/**
 * Dart Shelf route-surface D6448-ST bootstrap (cwl-api): hub-gold-dart-shelf → WebIR → CWL.
 * Route surface only — no Flutter / Dart Frog / Pipeline invent (D6447).
 */
...
export const HUB_DART_FLAGSHIP_KIND = "chrysalis.hub.dart-flagship";
export const HUB_DART_FLAGSHIP_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-dart-shelf");
const liftScript = join(scriptRoot, "scripts/hub-ingest/lift-to-webir.mjs");

/**
 * @param {string} [projectDir]
 */
export async function runDartFlagshipSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "lib", "hub_gold.dart");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_DART_FLAGSHIP_KIND,
      schemaVersion: HUB_DART_FLAGSHIP_SCHEMA_VERSION,
      ok: false,
      skip: "missing-hub-gold-dart",
      routeCount: 0,
      holeCount: null,
  ...
  const lift = spawnSync(process.execPath, [liftScript, root, "--language", "dart"], {
    cwd: scriptRoot,
    encoding: "utf8",
  ...
  const cwlMeta = await exportProjectMigrationCwl(root, { origin: "dart" });
  const webirPath = join(root, ".chrysalis", "hub.dart.webir.json");
  ...
  const { text } = renderCwlRoutes(routes, {
    header: "# Chrysalis Web Language — hub emit from dart (Shelf origin)",
    moduleName: "hub",
  });
  ...
  const verifySeed = join(scriptRoot, "fixtures/ci/hub-gold-dart-shelf-verify-for-status/summary.json");
...
async function main() {
  const report = await runDartFlagshipSmoke();
