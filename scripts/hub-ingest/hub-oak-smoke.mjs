#!/usr/bin/env node
/**
 * Smoke: hub-gold-oak Oak (Deno) TS dialect → WebIR hole-free (20 routes).
 * ORIGIN secondary — does not replace Express hub-flagship-express /
 * hub-flagship-typescript D6448-ST. Middleware stays honest holes.
 */
...
export const HUB_OAK_SMOKE_KIND = "chrysalis.hub.oak-smoke";
export const HUB_OAK_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-oak");
 ...
 */
export async function runOakSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_OAK_SMOKE_KIND,
      schemaVersion: HUB_OAK_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runOakSmoke();
