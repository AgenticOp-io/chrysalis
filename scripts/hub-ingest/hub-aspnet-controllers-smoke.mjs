#!/usr/bin/env node
/**
 * Smoke: hub-gold-aspnet-controllers ASP.NET controller dialect → WebIR hole-free (20 routes).
 * Does not replace hub-flagship-csharp Minimal API D6448-ST.
...
export const HUB_ASPNET_CONTROLLERS_SMOKE_KIND = "chrysalis.hub.aspnet-controllers-smoke";
export const HUB_ASPNET_CONTROLLERS_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-aspnet-controllers");
 ...
 */
export async function runAspnetControllersSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const controllerFile = join(root, "Controllers", "AppController.cs");
  if (!existsSync(controllerFile)) {
    return {
      kind: HUB_ASPNET_CONTROLLERS_SMOKE_KIND,
      schemaVersion: HUB_ASPNET_CONTROLLERS_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-controller-cs",
      routeCount: 0,
      holeCount: null,
  ...
  return {
    ...
    routeCount,
    holeCount,
    controllerRouteCount: liftReport.controllerRouteCount ?? null,
...
async function main() {
  const report = await runAspnetControllersSmoke();
