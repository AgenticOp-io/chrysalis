#!/usr/bin/env node
/**
 * Bootstrap hub-flagship-swift: Vapor 20-route origin → WebIR → CWL (D6448-ST cwl-api).
...
export const HUB_SWIFT_FLAGSHIP_KIND = "chrysalis.hub.swift-flagship";
export const HUB_SWIFT_FLAGSHIP_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-flagship-swift");
const liftScript = join(scriptRoot, "scripts/hub-ingest/lift-to-webir.mjs");

/**
 * @param {string} [projectDir]
 */
export async function runSwiftFlagshipSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const controller = join(root, "Sources", "App", "FlagshipRoutes.swift");
  if (!existsSync(controller)) {
    return {
      kind: HUB_SWIFT_FLAGSHIP_KIND,
      schemaVersion: HUB_SWIFT_FLAGSHIP_SCHEMA_VERSION,
      ok: false,
  ...
  const lift = spawnSync(process.execPath, [liftScript, root, "--language", "swift"], {
    cwd: scriptRoot,
    encoding: "utf8",
  ...
  const cwlMeta = await exportProjectMigrationCwl(root, { origin: "swift" });
  const webirPath = join(root, ".chrysalis", "hub.swift.webir.json");
  ...
  const { text } = renderCwlRoutes(routes, {
    header: "# Chrysalis Web Language — hub emit from swift",
    moduleName: "hub",
  });
  ...
  const verifySeed = join(scriptRoot, "fixtures/ci/hub-flagship-swift-verify-for-status/summary.json");
...
async function main() {
  const report = await runSwiftFlagshipSmoke();
