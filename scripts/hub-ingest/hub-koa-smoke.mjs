#!/usr/bin/env node
/**
 * Smoke: hub-gold-koa Koa TS dialect → WebIR hole-free (20 routes + pass-through middleware).
...
export const HUB_KOA_SMOKE_KIND = "chrysalis.hub.koa-smoke";
export const HUB_KOA_SMOKE_SCHEMA_VERSION = 2;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-koa");
 ...
 */
export async function runKoaSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_KOA_SMOKE_KIND,
      schemaVersion: HUB_KOA_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runKoaSmoke();
