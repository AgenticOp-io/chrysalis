#!/usr/bin/env node
/**
 * Smoke: hub-gold-elixir-plug Plug.Router → WebIR hole-free (20 routes).
 * Foundation only — not D6448-ST. No Phoenix LiveView invented (D6447).
 */
...
export const HUB_ELIXIR_SMOKE_KIND = "chrysalis.hub.elixir-smoke";
export const HUB_ELIXIR_SMOKE_SCHEMA_VERSION = 1;
 ...
 */
export async function runElixirSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "lib", "hub_gold", "router.ex");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_ELIXIR_SMOKE_KIND,
      schemaVersion: HUB_ELIXIR_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runElixirSmoke();
