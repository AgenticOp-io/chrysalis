#!/usr/bin/env node
/**
 * Smoke: hub-gold-jooby Jooby dialect → WebIR hole-free (20 routes).
...
export const HUB_JOOBY_SMOKE_KIND = "chrysalis.hub.jooby-smoke";
export const HUB_JOOBY_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-jooby");
 ...
 */
export async function runJoobySmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "HubApp.java");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_JOOBY_SMOKE_KIND,
      schemaVersion: HUB_JOOBY_SMOKE_SCHEMA_VERSION,
      ok: false,
  ...
  return {
    ...
    routeCount,
    holeCount,
    joobyRouteCount: liftReport.astRouteCount ?? null,
...
async function main() {
  const report = await runJoobySmoke();
