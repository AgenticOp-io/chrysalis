#!/usr/bin/env node
/**
 * Smoke: hub-gold-chi Chi dialect → WebIR hole-free (20 routes).
...
export const HUB_CHI_SMOKE_KIND = "chrysalis.hub.chi-smoke";
export const HUB_CHI_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-chi");
 ...
 */
export async function runChiSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_CHI_SMOKE_KIND,
      schemaVersion: HUB_CHI_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runChiSmoke();
