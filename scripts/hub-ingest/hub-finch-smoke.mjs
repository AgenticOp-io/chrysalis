#!/usr/bin/env node
/**
 * Smoke: hub-gold-finch Finch dialect → WebIR hole-free (20 routes).
 * Does not replace Akka hub-flagship-scala D6448-ST or Http4s secondary. G10051 / D6513.
 */
...
export const HUB_FINCH_SMOKE_KIND = "chrysalis.hub.finch-smoke";
export const HUB_FINCH_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-finch");
 ...
 */
export async function runFinchSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const routesFile = join(root, "src", "main", "scala", "hub", "FinchRoutes.scala");
  if (!existsSync(routesFile)) {
    return {
      kind: HUB_FINCH_SMOKE_KIND,
      schemaVersion: HUB_FINCH_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-finch-routes",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runFinchSmoke();
