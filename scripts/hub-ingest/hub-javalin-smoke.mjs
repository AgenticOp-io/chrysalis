#!/usr/bin/env node
/**
 * Smoke: hub-gold-javalin Javalin dialect → WebIR hole-free (20 routes).
...
export const HUB_JAVALIN_SMOKE_KIND = "chrysalis.hub.javalin-smoke";
export const HUB_JAVALIN_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-javalin");
 ...
 */
export async function runJavalinSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "HubApp.java");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_JAVALIN_SMOKE_KIND,
      schemaVersion: HUB_JAVALIN_SMOKE_SCHEMA_VERSION,
      ok: false,
  ...
  return {
    ...
    routeCount,
    holeCount,
    javalinRouteCount: liftReport.astRouteCount ?? null,
...
async function main() {
  const report = await runJavalinSmoke();
