#!/usr/bin/env node
/**
 * Smoke: hub-gold-beego Beego v2 functional dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST. G10045 / D6507.
 */
...
export const HUB_BEEGO_SMOKE_KIND = "chrysalis.hub.beego-smoke";
export const HUB_BEEGO_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-beego");
 ...
 */
export async function runBeegoSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_BEEGO_SMOKE_KIND,
      schemaVersion: HUB_BEEGO_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runBeegoSmoke();
