#!/usr/bin/env node
/**
 * Smoke: hub-gold-grape Grape dialect → WebIR hole-free (20 routes).
 * Does not replace Sinatra hub-flagship-ruby D6448-ST. G10032 / D6494.
 * Flat Grape reuses Sinatra peels — do not break hub:roda-smoke (G10022).
 */
...
export const HUB_GRAPE_SMOKE_KIND = "chrysalis.hub.grape-smoke";
export const HUB_GRAPE_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-grape");
 ...
 */
export async function runGrapeSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "app.rb");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_GRAPE_SMOKE_KIND,
      schemaVersion: HUB_GRAPE_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-rb",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runGrapeSmoke();
