#!/usr/bin/env node
/**
 * Bootstrap hub-flagship-vue: Vue SFC with Express 20-route API script → WebIR → CWL (D6448-ST cwl-api).
 * Honest SPA+API fixture via existing Vue→JS AST lift (hub-gold-vue-*); no invented product UI (D6447).
 */
...
export const HUB_VUE_FLAGSHIP_KIND = "chrysalis.hub.vue-flagship";
export const HUB_VUE_FLAGSHIP_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-flagship-vue");
const liftScript = join(scriptRoot, "scripts/hub-ingest/lift-to-webir.mjs");

/**
 * @param {string} [projectDir]
 */
export async function runVueFlagshipSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const controller = join(root, "App.vue");
  if (!existsSync(controller)) {
    return {
      kind: HUB_VUE_FLAGSHIP_KIND,
      schemaVersion: HUB_VUE_FLAGSHIP_SCHEMA_VERSION,
      ok: false,
  ...
  const lift = spawnSync(process.execPath, [liftScript, root, "--language", "vue"], {
    cwd: scriptRoot,
    encoding: "utf8",
  ...
  const cwlMeta = await exportProjectMigrationCwl(root, { origin: "vue" });
  const webirPath = join(root, ".chrysalis", "hub.vue.webir.json");
  ...
  const { text } = renderCwlRoutes(routes, {
    header: "# Chrysalis Web Language — hub emit from vue",
    moduleName: "hub",
  });
  ...
  const verifySeed = join(scriptRoot, "fixtures/ci/hub-flagship-vue-verify-for-status/summary.json");
...
async function main() {
  const report = await runVueFlagshipSmoke();
