#!/usr/bin/env node
/**
 * Bootstrap hub-flagship-csharp: ASP.NET Minimal API 20-route origin → WebIR → CWL (D6448-ST cwl-api).
 * Mirror of hub-flagship-express / python / go; no invented product UI (D6447).
 */
...
export const HUB_CSHARP_FLAGSHIP_KIND = "chrysalis.hub.csharp-flagship";
export const HUB_CSHARP_FLAGSHIP_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-flagship-csharp");
const liftScript = join(scriptRoot, "scripts/hub-ingest/lift-to-webir.mjs");

/**
 * @param {string} [projectDir]
 */
export async function runCsharpFlagshipSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const programCs = join(root, "Program.cs");
  if (!existsSync(programCs)) {
    return {
      kind: HUB_CSHARP_FLAGSHIP_KIND,
      schemaVersion: HUB_CSHARP_FLAGSHIP_SCHEMA_VERSION,
      ok: false,
      skip: "missing-program-cs",
      routeCount: 0,
      holeCount: null,
  ...
  const cwlMeta = await exportProjectMigrationCwl(root, { origin: "csharp" });
  ...
  const { text } = renderCwlRoutes(routes, {
    header: "# Chrysalis Web Language — hub emit from csharp",
    moduleName: "hub",
  });
  ...
  const verifySeed = join(scriptRoot, "fixtures/ci/hub-flagship-csharp-verify-for-status/summary.json");
...
async function main() {
  const report = await runCsharpFlagshipSmoke();
