...
export async function runCwlPinSmoke(opts = {}) {
  ...
  // Tip floor for CWL thin-emit reverse + foreach/else (LANGUAGE_VERSION / 1.0.10).
  ...
  const tipOk =
    ...
    (parts[0] > 1 ||
      (parts[0] === 1 && parts[1] > 0) ||
      (parts[0] === 1 && parts[1] === 0 && parts[2] >= 10));
  checks.push({
    id: "cwl-1.0.10-tip-floor",
    ok: tipOk,
    detail: `VERSION=${version} (expect >= 1.0.10 after foreach/else + thin emit reverse)`,
  });
