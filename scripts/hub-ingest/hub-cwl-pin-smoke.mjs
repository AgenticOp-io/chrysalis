...
export async function runCwlPinSmoke(opts = {}) {
  ...
  // Tip floor for CWL Helix DNA seed parity + control-lower sync (LANGUAGE_VERSION / 1.0.51).
  ...
  const tipOk =
    ...
    (parts[0] > 1 ||
      (parts[0] === 1 && parts[1] > 0) ||
      (parts[0] === 1 && parts[1] === 0 && parts[2] >= 51));
  checks.push({
    id: "cwl-1.0.51-tip-floor",
    ok: tipOk,
    detail: `VERSION=${version} (expect >= 1.0.51 auth.require cookie + db table + mail template + cors methods + cache.max-age (golds 55–59))`,
  });
