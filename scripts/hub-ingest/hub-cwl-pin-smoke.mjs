...
export async function runCwlPinSmoke(opts = {}) {
  ...
  // Tip floor for CWL Helix DNA seed parity + control-lower sync (LANGUAGE_VERSION / 1.0.22).
  ...
  const tipOk =
    ...
    (parts[0] > 1 ||
      (parts[0] === 1 && parts[1] > 0) ||
      (parts[0] === 1 && parts[1] === 0 && parts[2] >= 22));
  checks.push({
    id: "cwl-1.0.22-tip-floor",
    ok: tipOk,
    detail: `VERSION=${version} (expect >= 1.0.22 SSE single-shot stream surface (gold 32))`,
  });
