...
export async function runCwlPinSmoke(opts = {}) {
  ...
  // Tip floor for CWL CI WebIR + emit catalog (LANGUAGE_VERSION / 1.0.13).
  ...
  const tipOk =
    ...
    (parts[0] > 1 ||
      (parts[0] === 1 && parts[1] > 0) ||
      (parts[0] === 1 && parts[1] === 0 && parts[2] >= 13));
  checks.push({
    id: "cwl-1.0.13-tip-floor",
    ok: tipOk,
    detail: `VERSION=${version} (expect >= 1.0.13 after emit catalog/CLI polish)`,
  });
