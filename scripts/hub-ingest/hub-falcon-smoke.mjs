#!/usr/bin/env node
/**
 * Smoke: hub-gold-falcon Falcon dialect → WebIR hole-free (20 routes).
...
export const HUB_FALCON_SMOKE_KIND = "chrysalis.hub.falcon-smoke";
export const HUB_FALCON_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-falcon");
 ...
 */
export async function runFalconSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "app.py");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_FALCON_SMOKE_KIND,
      schemaVersion: HUB_FALCON_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-py",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runFalconSmoke();
