#!/usr/bin/env node
/**
 * Smoke: hub-gold-deno-serve Deno.serve pathname+method dialect → WebIR
 * hole-free (20 routes). ORIGIN secondary — does not replace Express/TS
 * D6448-ST. Oak / Bun.serve / itty / CF Workers stay green (G10118 / D6543).
 */
...
export const HUB_DENO_SERVE_SMOKE_KIND = "chrysalis.hub.deno-serve-smoke";
export const HUB_DENO_SERVE_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-deno-serve");
 ...
 */
export async function runDenoServeSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_DENO_SERVE_SMOKE_KIND,
      schemaVersion: HUB_DENO_SERVE_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runDenoServeSmoke();
