#!/usr/bin/env node
/**
 * Smoke: hub-gold-gorilla Gorilla mux dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST. G10018 / D6480.
 */
...
export const HUB_GORILLA_SMOKE_KIND = "chrysalis.hub.gorilla-smoke";
export const HUB_GORILLA_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-gorilla");
 ...
 */
export async function runGorillaSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_GORILLA_SMOKE_KIND,
      schemaVersion: HUB_GORILLA_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runGorillaSmoke();
