#!/usr/bin/env node
/**
 * Smoke: hub-gold-sparkjava Spark Java dialect → WebIR hole-free (20 routes).
...
export const HUB_SPARKJAVA_SMOKE_KIND = "chrysalis.hub.sparkjava-smoke";
export const HUB_SPARKJAVA_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-sparkjava");
 ...
 */
export async function runSparkjavaSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "HubApp.java");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_SPARKJAVA_SMOKE_KIND,
      schemaVersion: HUB_SPARKJAVA_SMOKE_SCHEMA_VERSION,
      ok: false,
  ...
  return {
    ...
    routeCount,
    holeCount,
    sparkjavaRouteCount: liftReport.astRouteCount ?? null,
...
async function main() {
  const report = await runSparkjavaSmoke();
