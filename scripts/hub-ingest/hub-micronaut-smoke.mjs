#!/usr/bin/env node
/**
 * Smoke: hub-gold-micronaut Micronaut dialect → WebIR hole-free (20 routes).
...
export const HUB_MICRONAUT_SMOKE_KIND = "chrysalis.hub.micronaut-smoke";
export const HUB_MICRONAUT_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-micronaut");
 ...
 */
export async function runMicronautSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const controllerFile = join(root, "src", "HubController.java");
  if (!existsSync(controllerFile)) {
    return {
      kind: HUB_MICRONAUT_SMOKE_KIND,
      schemaVersion: HUB_MICRONAUT_SMOKE_SCHEMA_VERSION,
      ok: false,
  ...
  return {
    ...
    routeCount,
    holeCount,
    micronautRouteCount: liftReport.astRouteCount ?? null,
...
async function main() {
  const report = await runMicronautSmoke();
