...
export function runPausedAndMaintenanceDocGate() {
  ...
  const docOk = isPhase46ProgramClosed()
    ...
    ? text.includes("G7990") &&
      ...
      text.includes("D6272") &&
      text.includes("Do not treat closed program tables")
    : isWispProductionCompletionProgramActive()
    ? text.includes("G7900") &&
      text.includes("G7990") &&
      text.includes("WISP-PRODUCTION-COMPLETION-PROGRAM.md") &&
      text.includes("D6272") &&
      text.includes("Do not treat closed program tables")
...
export function runStrategicPlanMaintenanceDefaultQueueGate() {
  ...
  if (isPhase45ProgramActive()) {
    ...
    const ok =
      ...
      text.includes("D6336") &&
      text.includes("G9170") &&
      text.includes("## 12 (archived) — Default queue (Phase 45 active") &&
      text.includes("PAUSED-AND-MAINTENANCE.md");
  ...
  if (isWispProductionCompletionProgramClosed()) {
    ...
    const ok =
      ...
      text.includes("D6272") &&
      text.includes("PAUSED-AND-MAINTENANCE.md");
    return { ok, wispProductionCompletionClosedOk: ok };
  }
  if (isWispProductionCompletionProgramActive()) {
    const path = join(scriptRoot, "docs/STRATEGIC-PLAN.md");
    if (!existsSync(path)) return { ok: false, skip: "missing-strategic-plan" };
    const text = readFileSync(path, "utf8");
    const ok =
      text.includes("Phase 29") &&
      text.includes("G7900") &&
      text.includes("G7990") &&
      text.includes("D6272") &&
      text.includes("PAUSED-AND-MAINTENANCE.md");
    return { ok, wispProductionCompletionActiveOk: ok };
  }
...
let cachedPhase44ProgramCloseResult = null;
let cachedPhase45ProgramCloseResult = null;
...
export function resetFullMatrixOracleCloseCacheForTests() {
  ...
  cachedPhase44ProgramCloseResult = null;
  cachedPhase45ProgramCloseResult = null;
}

/** G9141 — Phase 44 closed governance (post G9140; subordinate G8790). */
export async function runPhase44ClosedGovernanceGate(_opts = {}) {
  const skipClose =
    _opts.skipPhase44ProgramClose === true ||
    _opts.skipCloseSmoke === true ||
    process.env.CHRYSALIS_STRATEGIC_PLAN_SKIP_PHASE44 === "1";
  const skipFullMatrix =
    _opts.skipFullMatrixOracleClose === true ||
    process.env.CHRYSALIS_STRATEGIC_PLAN_SKIP_FULL_MATRIX_ORACLE === "1";
  const doc = runPhase44ProgramDocGate();
  ...
  let fullMatrixCloseOk = true;
  let fullMatrixClose = null;
  let fullMatrixCloseCached = false;
  if (!skipClose) {
    if (_opts.phase44ProgramClose != null) {
      close = _opts.phase44ProgramClose;
      closeOk = close.ok === true;
      closeCached = true;
    } else if (cachedPhase44ProgramCloseResult != null) {
      close = cachedPhase44ProgramCloseResult;
      closeOk = close.ok === true;
      closeCached = true;
    } else {
      const { runPhase44ProgramCloseGate } = await import("./hub-phase44-program-close-smoke.mjs");
      close = await runPhase44ProgramCloseGate(_opts);
      cachedPhase44ProgramCloseResult = close;
      closeOk = close.ok === true;
    }
  }
  if (!skipFullMatrix) {
    if (_opts.fullMatrixOracleClose != null) {
      fullMatrixClose = _opts.fullMatrixOracleClose;
      fullMatrixCloseOk = fullMatrixClose.ok === true;
      fullMatrixCloseCached = true;
    } else if (cachedFullMatrixOracleCloseResult != null) {
      fullMatrixClose = cachedFullMatrixOracleCloseResult;
      fullMatrixCloseOk = fullMatrixClose.ok === true;
      fullMatrixCloseCached = true;
    } else {
      const { runFullMatrixOracleCloseGate } = await import("./hub-full-matrix-oracle-close-smoke.mjs");
      fullMatrixClose = await runFullMatrixOracleCloseGate({ ..._opts, skipMaintenance: true });
      cachedFullMatrixOracleCloseResult = fullMatrixClose;
      fullMatrixCloseOk = fullMatrixClose.ok === true;
    }
  }
  const ok =
...
export async function runPhase45ClosedGovernanceGate(_opts = {}) {
  const skipClose =
    _opts.skipPhase45ProgramClose === true ||
    _opts.skipCloseSmoke === true ||
    process.env.CHRYSALIS_STRATEGIC_PLAN_SKIP_PHASE45 === "1";
  const doc = runPhase45ProgramDocGate();
  const paused = runPausedAndMaintenanceDocGate();
  const strategicPlan = runStrategicPlanMaintenanceDefaultQueueGate();
  const roadmap = runRoadmapMaintenanceDefaultQueueGate();
  ...
  let close = null;
  if (!skipClose) {
    if (_opts.phase45ProgramClose != null) {
      close = _opts.phase45ProgramClose;
      closeOk = close.ok === true;
    } else if (cachedPhase45ProgramCloseResult != null) {
      close = cachedPhase45ProgramCloseResult;
      closeOk = close.ok === true;
    } else {
      const { runPhase45ProgramCloseGate } = await import("./hub-phase45-program-close-smoke.mjs");
      close = await runPhase45ProgramCloseGate(_opts);
      cachedPhase45ProgramCloseResult = close;
      closeOk = close.ok === true;
    }
  }
  const ok =
...
export async function runMaintenanceModeGovernanceGate(_opts = {}) {
  ...
  if (isWispProductionCompletionProgramActive()) {
    return runWispProductionCompletionActiveGovernanceGate(_opts);
  }
  if (isWispProductionPocProgramClosed()) {
...
/** G7134 — IR helper lifting B76 json_encode((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB76JsonEncodeInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_json_encode.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m76.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b76-json_encode-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok = r.status === 0 && helper.includes("json_encode($label)") && doc.includes("runIrHelperLiftingB76JsonEncodeInlineGate") && doc.includes("B76 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7135 — IR helper lifting B77 json_decode((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB77JsonDecodeInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_json_decode.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m77.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b77-json_decode-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok = r.status === 0 && helper.includes("json_decode($label)") && doc.includes("runIrHelperLiftingB77JsonDecodeInlineGate") && doc.includes("B77 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7136 — IR helper lifting B78 md5((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB78Md5InlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_md5.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m78.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b78-md5-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok = r.status === 0 && helper.includes("md5($label)") && doc.includes("runIrHelperLiftingB78Md5InlineGate") && doc.includes("B78 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7137 — IR helper lifting B79 sha1((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB79Sha1InlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_sha1.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m79.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b79-sha1-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok = r.status === 0 && helper.includes("sha1($label)") && doc.includes("runIrHelperLiftingB79Sha1InlineGate") && doc.includes("B79 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7138 — IR helper lifting B80 base64_encode((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB80Base64EncodeInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_base64_encode.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m80.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b80-base64_encode-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("base64_encode($label)") &&
    doc.includes("runIrHelperLiftingB80Base64EncodeInlineGate") &&
    doc.includes("B80 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7139 — IR helper lifting B81 base64_decode((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB81Base64DecodeInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_base64_decode.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m81.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b81-base64_decode-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("base64_decode($label)") &&
    doc.includes("runIrHelperLiftingB81Base64DecodeInlineGate") &&
    doc.includes("B81 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7143 — IR helper lifting B82 bin2hex((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB82Bin2hexInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_bin2hex.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m82.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b82-bin2hex-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("bin2hex($label)") &&
    doc.includes("runIrHelperLiftingB82Bin2hexInlineGate") &&
    doc.includes("B82 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7144 — IR helper lifting B83 preg_quote((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB83PregQuoteInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_preg_quote.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m83.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b83-preg_quote-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("preg_quote($label)") &&
    doc.includes("runIrHelperLiftingB83PregQuoteInlineGate") &&
    doc.includes("B83 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7145 — IR helper lifting B84 parse_url((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB84ParseUrlInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_parse_url.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m84.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b84-parse_url-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("parse_url($label)") &&
    doc.includes("runIrHelperLiftingB84ParseUrlInlineGate") &&
    doc.includes("B84 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7146 — IR helper lifting B85 basename((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB85BasenameInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_basename.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m85.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b85-basename-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("basename($label)") &&
    doc.includes("runIrHelperLiftingB85BasenameInlineGate") &&
    doc.includes("B85 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7147 — IR helper lifting B86 dirname((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB86DirnameInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_dirname.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m86.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b86-dirname-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("dirname($label)") &&
    doc.includes("runIrHelperLiftingB86DirnameInlineGate") &&
    doc.includes("B86 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7148 — IR helper lifting B87 gettype((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB87GettypeInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_gettype.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m87.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b87-gettype-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("gettype($label)") &&
    doc.includes("runIrHelperLiftingB87GettypeInlineGate") &&
    doc.includes("B87 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7149 — IR helper lifting B88 is_callable((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB88IsCallableInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_is_callable.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m88.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b88-is_callable-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("is_callable($label)") &&
    doc.includes("runIrHelperLiftingB88IsCallableInlineGate") &&
    doc.includes("B88 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7152 — IR helper lifting B89 is_resource((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB89IsResourceInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_is_resource.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m89.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b89-is_resource-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("is_resource($label)") &&
    doc.includes("runIrHelperLiftingB89IsResourceInlineGate") &&
    doc.includes("B89 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7153 — IR helper lifting B90 ord((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB90OrdInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_ord.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m90.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b90-ord-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok = r.status === 0 && helper.includes("ord($code)") && doc.includes("runIrHelperLiftingB90OrdInlineGate") && doc.includes("B90 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7154 — IR helper lifting B91 chr((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB91ChrInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_chr.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m91.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b91-chr-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok = r.status === 0 && helper.includes("chr($code)") && doc.includes("runIrHelperLiftingB91ChrInlineGate") && doc.includes("B91 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7155 — IR helper lifting B92 preg_match(, lit) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB92PregMatchInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_preg_match.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m92.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b92-preg_match-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("preg_match('/\\d+/', $label)") &&
    doc.includes("runIrHelperLiftingB92PregMatchInlineGate") &&
    doc.includes("B92 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7156 — IR helper lifting B93 hash(, lit) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB93HashInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_hash.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m93.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b93-hash-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("hash('sha256', $label)") &&
    doc.includes("runIrHelperLiftingB93HashInlineGate") &&
    doc.includes("B93 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7157 — IR helper lifting B94 sprintf(, lit) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB94SprintfInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_sprintf.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m94.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b94-sprintf-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("sprintf('%s', $label)") &&
    doc.includes("runIrHelperLiftingB94SprintfInlineGate") &&
    doc.includes("B94 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7158 — IR helper lifting B95 number_format(, lit) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB95NumberFormatInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_number_format2.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m95.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b95-number_format-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("number_format($label, 2)") &&
    doc.includes("runIrHelperLiftingB95NumberFormatInlineGate") &&
    doc.includes("B95 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7159 — IR helper lifting B96 implode(lit, ...) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB96ImplodeInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_implode.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m96.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b96-implode-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("implode(',', $arr)") &&
    doc.includes("runIrHelperLiftingB96ImplodeInlineGate") &&
    doc.includes("B96 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7160 — IR helper lifting B97 preg_replace(lit, lit, ...) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB97PregReplaceInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_preg_replace.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m97.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b97-preg_replace-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("preg_replace('/a/', 'b', $label)") &&
    doc.includes("runIrHelperLiftingB97PregReplaceInlineGate") &&
    doc.includes("B97 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7161 — IR helper lifting B98 preg_split(, lit) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB98PregSplitInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_preg_split.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m98.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b98-preg_split-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("preg_split('/,/', $label)") &&
    doc.includes("runIrHelperLiftingB98PregSplitInlineGate") &&
    doc.includes("B98 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7162 — IR helper lifting B99 hexdec((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB99HexdecInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_hexdec.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m99.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b99-hexdec-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("hexdec($label)") &&
    doc.includes("runIrHelperLiftingB99HexdecInlineGate") &&
    doc.includes("B99 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7163 — IR helper lifting B100 dechex((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB100DechexInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_dechex.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m100.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b100-dechex-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("dechex($active)") &&
    doc.includes("runIrHelperLiftingB100DechexInlineGate") &&
    doc.includes("B100 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7164 — IR helper lifting B101 strval((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB101StrvalInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_strval.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m101.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b101-strval-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("strval($active)") &&
    doc.includes("runIrHelperLiftingB101StrvalInlineGate") &&
    doc.includes("B101 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7165 — IR helper lifting B102 filter_var(, lit) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB102FilterVarInlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_filter_var.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m102.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b102-filter_var-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("filter_var($label, 513)") &&
    doc.includes("runIrHelperLiftingB102FilterVarInlineGate") &&
    doc.includes("B102 v0");
  return { ok, vitestOk: r.status === 0 };
}

/** G7166 — IR helper lifting B103 crc32((, ...)) formal assign inline (CWL language v1.1). */
export function runIrHelperLiftingB103Crc32InlineGate() {
  const helperPath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/lib/sql_param_crc32.php");
  const routePath = join(scriptRoot, "fixtures/lift-helper-sql-param-inline/pages/show_m103.php");
  const docPath = join(scriptRoot, "docs/IR-HELPER-LIFTING.md");
  if (!existsSync(helperPath) || !existsSync(routePath) || !existsSync(docPath)) {
    return { ok: false, skip: "missing-b103-crc32-fixture" };
  }
  const helper = readFileSync(helperPath, "utf8");
  const doc = readFileSync(docPath, "utf8");
  const r = spawnSync(
  ...
  const ok =
    r.status === 0 &&
    helper.includes("crc32($label)") &&
    doc.includes("runIrHelperLiftingB103Crc32InlineGate") &&
    doc.includes("B103 v0");
  return { ok, vitestOk: r.status === 0 };
}
...
const wispProductionCompletionProgramDocPath = join(
  scriptRoot,
  "docs/WISP-PRODUCTION-COMPLETION-PROGRAM.md",
);

/** @returns {boolean} WISP production completion program is active (G7900, D6272). */
export function isWispProductionCompletionProgramActive() {
  if (isWispProductionCompletionProgramClosed()) return false;
  if (!existsSync(wispProductionCompletionProgramDocPath)) return false;
  const text = readFileSync(wispProductionCompletionProgramDocPath, "utf8");
  return text.includes("**Status:** **active**") && text.includes("G7900");
}

/** @returns {boolean} WISP production completion program closed (G7990). */
export function isWispProductionCompletionProgramClosed() {
  if (!existsSync(wispProductionCompletionProgramDocPath)) return false;
  const text = readFileSync(wispProductionCompletionProgramDocPath, "utf8");
  return text.includes("Program closed") && text.includes("G7990");
}

/** G7901 — WISP production completion active governance (post G7900 entry). */
export async function runWispProductionCompletionActiveGovernanceGate(_opts = {}) {
  const { runWispProductionCompletionProgramEntryGate } = await import(
    "./hub-wisp-production-completion-program-entry-smoke.mjs"
  );
  const entry = await runWispProductionCompletionProgramEntryGate(_opts);
  const paused = runPausedAndMaintenanceDocGate();
  const strategicPlan = runStrategicPlanMaintenanceDefaultQueueGate();
  const roadmap = runRoadmapMaintenanceDefaultQueueGate();
  const taxonomy = runCwlSurfaceTaxonomyDocGate();
  ...
  return {
    ...
    roadmapOk: roadmap.ok === true,
    taxonomyOk: taxonomy.ok === true,
    mode: "wisp-production-completion-active",
  };
}

/** G7991 — WISP production completion closed governance. */
export async function runWispProductionCompletionClosedGovernanceGate(_opts = {}) {
  const skipGoldVerify =
    _opts.skipGoldVerify === true || process.env.CHRYSALIS_STRATEGIC_PLAN_SKIP_FLAGSHIP_GOLD === "1";
  const { runWispProductionCompletionCloseGate, runWispProductionCompletionDocGate } = await import(
    "./hub-wisp-production-completion-close-smoke.mjs"
  );
  const close =
    _opts.skipCloseSmoke === true
      ? runWispProductionCompletionDocGate()
      : await runWispProductionCompletionCloseGate({ ..._opts, skipGoldVerify });
  const paused = runPausedAndMaintenanceDocGate();
  const strategicPlan = runStrategicPlanMaintenanceDefaultQueueGate();
  const roadmap = runRoadmapMaintenanceDefaultQueueGate();
  const taxonomy = runCwlSurfaceTaxonomyDocGate();
  ...
  return {
    ...
    roadmapOk: roadmap.ok === true,
    taxonomyOk: taxonomy.ok === true,
    mode: "wisp-production-completion-closed",
  };
}
...
function summarizeMaintenanceModeGovernance(governance) {
  if (!governance || typeof governance !== "object") return governance ?? null;
  if (governance.mode === "phase44-closed") {
    return {
      ok: governance.ok === true,
      closeOk: governance.closeOk === true,
      closeCached: governance.closeCached === true,
      fullMatrixCloseOk: governance.fullMatrixCloseOk === true,
      fullMatrixCloseCached: governance.fullMatrixCloseCached === true,
  ...
  return summarizeFullMatrixOracleGovernance(governance);
}

/** @param {Record<string, unknown> | null | undefined} close */
function summarizePhase44ProgramClose(close) {
  if (!close || typeof close !== "object") return close ?? null;
  const buildSlice = /** @type {Record<string, unknown>} */ (close.buildSlice);
  return {
    ok: close.ok === true,
    programHonest: close.programHonest === true,
    programClosed: close.programClosed === true,
    buildSliceOk: buildSlice?.ok === true,
    oracleProductCount: /** @type {Record<string, unknown>} */ (close.census)?.oracleProductCount ?? null,
  };
}
...
function summarizePhase10ReinforcementGate(reinforcement) {
  ...
  return {
    ...
    governance: summarizeMaintenanceModeGovernance(
...
function summarizePhase10ArchiveGate(archive) {
  ...
  return {
    ...
    maintenance: summarizeMaintenanceModeGovernance(
...
export async function runMaintenanceProgramCompleteGate(opts = {}) {
  resetFullMatrixOracleCloseCacheForTests();
  let gateOpts = opts;
  const skipClose =
    opts.skipFullMatrixOracleClose === true ||
    process.env.CHRYSALIS_STRATEGIC_PLAN_SKIP_FULL_MATRIX_ORACLE === "1";
  const skipPhase44 =
    opts.skipPhase44ProgramClose === true ||
    process.env.CHRYSALIS_STRATEGIC_PLAN_SKIP_PHASE44 === "1";
  const skipPhase45 =
    opts.skipPhase45ProgramClose === true ||
    process.env.CHRYSALIS_STRATEGIC_PLAN_SKIP_PHASE45 === "1";
  if (isPhase45ProgramClosed() && !skipPhase45 && opts.phase45ProgramClose == null) {
    const { runPhase45ProgramCloseGate } = await import("./hub-phase45-program-close-smoke.mjs");
    const phase45ProgramClose = await runPhase45ProgramCloseGate({ ...opts });
    cachedPhase45ProgramCloseResult = phase45ProgramClose;
    gateOpts = { ...gateOpts, phase45ProgramClose };
  }
  if (isPhase44ProgramClosed() && !skipPhase44 && opts.phase44ProgramClose == null) {
    const { runPhase44ProgramCloseGate } = await import("./hub-phase44-program-close-smoke.mjs");
    const phase44ProgramClose = await runPhase44ProgramCloseGate({ ...opts });
    cachedPhase44ProgramCloseResult = phase44ProgramClose;
    gateOpts = { ...gateOpts, phase44ProgramClose };
  }
  if (isFullMatrixOracleProgramClosed() && !skipClose && gateOpts.fullMatrixOracleClose == null) {
    const { runFullMatrixOracleCloseGate } = await import("./hub-full-matrix-oracle-close-smoke.mjs");
    ...
    gateOpts = { ...gateOpts, fullMatrixOracleClose };
  }
  ...
  return {
    ...
    programClosed: isPhase10ProductionParityClosed(),
    phase44CloseOk:
      gateOpts.phase44ProgramClose?.ok === true || cachedPhase44ProgramCloseResult?.ok === true,
    phase44Close: summarizePhase44ProgramClose(
      gateOpts.phase44ProgramClose ?? cachedPhase44ProgramCloseResult,
    ),
    fullMatrixCloseOk:
    ...
    maintenance: summarizeMaintenanceModeGovernance(maintenance),
  };
}
