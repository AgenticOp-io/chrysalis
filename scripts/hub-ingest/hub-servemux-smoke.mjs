#!/usr/bin/env node
/**
 * Smoke: hub-gold-servemux Go 1.22+ net/http ServeMux dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST. G10030 / D6492.
 */
...
export const HUB_SERVEMUX_SMOKE_KIND = "chrysalis.hub.servemux-smoke";
export const HUB_SERVEMUX_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-servemux");
 ...
 */
export async function runServeMuxSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_SERVEMUX_SMOKE_KIND,
      schemaVersion: HUB_SERVEMUX_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runServeMuxSmoke();
