#!/usr/bin/env node
/**
 * Smoke: hub-gold-fiber Fiber dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST. G10017 / D6479.
 */
...
export const HUB_FIBER_SMOKE_KIND = "chrysalis.hub.fiber-smoke";
export const HUB_FIBER_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-fiber");
 ...
 */
export async function runFiberSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_FIBER_SMOKE_KIND,
      schemaVersion: HUB_FIBER_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runFiberSmoke();
