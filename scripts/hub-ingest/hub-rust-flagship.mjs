#!/usr/bin/env node
/**
 * Bootstrap hub-flagship-rust: Actix Web 20-route origin → WebIR → CWL (D6448-ST cwl-api).
 * Mirror of hub-flagship-kotlin / scala / express; no invented product UI (D6447).
 */
...
export const HUB_RUST_FLAGSHIP_KIND = "chrysalis.hub.rust-flagship";
export const HUB_RUST_FLAGSHIP_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-flagship-rust");
const liftScript = join(scriptRoot, "scripts/hub-ingest/lift-to-webir.mjs");

/**
 * @param {string} [projectDir]
 */
export async function runRustFlagshipSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const controller = join(root, "src", "main.rs");
  if (!existsSync(controller)) {
    return {
      kind: HUB_RUST_FLAGSHIP_KIND,
      schemaVersion: HUB_RUST_FLAGSHIP_SCHEMA_VERSION,
      ok: false,
  ...
  const lift = spawnSync(process.execPath, [liftScript, root, "--language", "rust"], {
    cwd: scriptRoot,
    encoding: "utf8",
  ...
  const cwlMeta = await exportProjectMigrationCwl(root, { origin: "rust" });
  const webirPath = join(root, ".chrysalis", "hub.rust.webir.json");
  ...
  const { text } = renderCwlRoutes(routes, {
    header: "# Chrysalis Web Language — hub emit from rust",
    moduleName: "hub",
  });
  ...
  const verifySeed = join(scriptRoot, "fixtures/ci/hub-flagship-rust-verify-for-status/summary.json");
...
async function main() {
  const report = await runRustFlagshipSmoke();
