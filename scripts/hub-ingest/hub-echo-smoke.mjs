#!/usr/bin/env node
/**
 * Smoke: hub-gold-echo Echo dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST.
 */
...
export const HUB_ECHO_SMOKE_KIND = "chrysalis.hub.echo-smoke";
export const HUB_ECHO_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-echo");
 ...
 */
export async function runEchoSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_ECHO_SMOKE_KIND,
      schemaVersion: HUB_ECHO_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runEchoSmoke();
