#!/usr/bin/env node
/**
 * Smoke: hub-gold-lumen Lumen / Laravel-router dialect → WebIR hole-free (20 routes).
 * Does not replace Laravel/Symfony/plain-php D6448-ST. G10049 / D6511.
 */
...
export const HUB_LUMEN_SMOKE_KIND = "chrysalis.hub.lumen-smoke";
export const HUB_LUMEN_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-lumen");
 ...
 */
export async function runLumenSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const routesFile = join(root, "routes", "web.php");
  if (!existsSync(routesFile)) {
    return {
      kind: HUB_LUMEN_SMOKE_KIND,
      schemaVersion: HUB_LUMEN_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-routes-web-php",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runLumenSmoke();
