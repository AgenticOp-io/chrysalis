#!/usr/bin/env node
/**
 * Smoke: hub-gold-tapir Tapir endpoint dialect → WebIR hole-free (20 routes).
 * Does not replace Akka hub-flagship-scala D6448-ST or Http4s/Finch secondaries.
 * G10119 / D6544 — no jsonBody/plainBody invent; serverLogicSuccess Map/lit only.
 */
...
export const HUB_TAPIR_SMOKE_KIND = "chrysalis.hub.tapir-smoke";
export const HUB_TAPIR_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-tapir");
 ...
 */
export async function runTapirSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const routesFile = join(root, "src", "main", "scala", "hub", "TapirRoutes.scala");
  if (!existsSync(routesFile)) {
    return {
      kind: HUB_TAPIR_SMOKE_KIND,
      schemaVersion: HUB_TAPIR_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-tapir-routes",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runTapirSmoke();
