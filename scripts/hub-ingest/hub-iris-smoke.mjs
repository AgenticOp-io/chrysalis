#!/usr/bin/env node
/**
 * Smoke: hub-gold-iris Iris dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST. G10038 / D6500.
 */
...
export const HUB_IRIS_SMOKE_KIND = "chrysalis.hub.iris-smoke";
export const HUB_IRIS_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-iris");
 ...
 */
export async function runIrisSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_IRIS_SMOKE_KIND,
      schemaVersion: HUB_IRIS_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runIrisSmoke();
