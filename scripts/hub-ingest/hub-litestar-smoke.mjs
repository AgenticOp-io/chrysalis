#!/usr/bin/env node
/**
 * Smoke: hub-gold-litestar Litestar dialect → WebIR hole-free (20 routes).
...
export const HUB_LITESTAR_SMOKE_KIND = "chrysalis.hub.litestar-smoke";
export const HUB_LITESTAR_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-litestar");
 ...
 */
export async function runLitestarSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "app.py");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_LITESTAR_SMOKE_KIND,
      schemaVersion: HUB_LITESTAR_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-py",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runLitestarSmoke();
