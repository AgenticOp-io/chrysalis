#!/usr/bin/env node
/**
 * Smoke: hub-gold-restify Restify TS dialect → WebIR hole-free (20 routes + pre/use pass-through).
...
export const HUB_RESTIFY_SMOKE_KIND = "chrysalis.hub.restify-smoke";
export const HUB_RESTIFY_SMOKE_SCHEMA_VERSION = 2;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-restify");
const liftScript = join(scriptRoot, "scripts/hub-ingest/lift-to-webir.mjs");

const EXPECT_ROUTES = 20;
/** Pass-through `server.pre` + `server.use` preset floor (G9959). */
const EXPECT_MIDDLEWARE = 2;

/**
 * @param {string} [projectDir]
 */
export async function runRestifySmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_RESTIFY_SMOKE_KIND,
      schemaVersion: HUB_RESTIFY_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runRestifySmoke();
