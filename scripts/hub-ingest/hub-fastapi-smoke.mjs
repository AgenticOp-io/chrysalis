#!/usr/bin/env node
/**
 * Smoke: hub-gold-fastapi FastAPI dialect → WebIR hole-free (20 routes).
...
export const HUB_FASTAPI_SMOKE_KIND = "chrysalis.hub.fastapi-smoke";
export const HUB_FASTAPI_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-fastapi");
 ...
 */
export async function runFastapiSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "app.py");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_FASTAPI_SMOKE_KIND,
      schemaVersion: HUB_FASTAPI_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-py",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runFastapiSmoke();
