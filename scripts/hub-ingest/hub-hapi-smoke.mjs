#!/usr/bin/env node
/**
 * Smoke: hub-gold-hapi Hapi TS dialect → WebIR hole-free (20 routes).
...
export const HUB_HAPI_SMOKE_KIND = "chrysalis.hub.hapi-smoke";
export const HUB_HAPI_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-hapi");
 ...
 */
export async function runHapiSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_HAPI_SMOKE_KIND,
      schemaVersion: HUB_HAPI_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runHapiSmoke();
