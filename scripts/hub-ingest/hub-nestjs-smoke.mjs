#!/usr/bin/env node
/**
 * Smoke: hub-gold-nestjs NestJS TS decorator dialect → WebIR hole-free (20 routes).
 * Does not replace Express hub-flagship-express / hub-flagship-typescript D6448-ST.
 * Distinct from any Nest emit backend (matrix output aspiration only).
 */
...
export const HUB_NESTJS_SMOKE_KIND = "chrysalis.hub.nestjs-smoke";
export const HUB_NESTJS_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-nestjs");
 ...
 */
export async function runNestjsSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_NESTJS_SMOKE_KIND,
      schemaVersion: HUB_NESTJS_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
  ...
  return {
    ...
    routeCount,
    holeCount,
    nestRouteCount: liftReport.nestRouteCount ?? null,
...
async function main() {
  const report = await runNestjsSmoke();
