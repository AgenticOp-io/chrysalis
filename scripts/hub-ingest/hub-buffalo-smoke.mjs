#!/usr/bin/env node
/**
 * Smoke: hub-gold-buffalo Buffalo dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST. G10055 / D6517.
 */
...
export const HUB_BUFFALO_SMOKE_KIND = "chrysalis.hub.buffalo-smoke";
export const HUB_BUFFALO_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-buffalo");
 ...
 */
export async function runBuffaloSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_BUFFALO_SMOKE_KIND,
      schemaVersion: HUB_BUFFALO_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runBuffaloSmoke();
