#!/usr/bin/env node
/**
 * Smoke: hub-gold-martini Martini dialect → WebIR hole-free (20 routes).
 * Does not replace Gin hub-flagship-go D6448-ST. G10056 / D6518.
 */
...
export const HUB_MARTINI_SMOKE_KIND = "chrysalis.hub.martini-smoke";
export const HUB_MARTINI_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-martini");
 ...
 */
export async function runMartiniSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "main.go");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_MARTINI_SMOKE_KIND,
      schemaVersion: HUB_MARTINI_SMOKE_SCHEMA_VERSION,
      ok: false,
...
async function main() {
  const report = await runMartiniSmoke();
