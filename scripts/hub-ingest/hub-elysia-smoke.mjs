 ...
 */
export async function runElysiaSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_ELYSIA_SMOKE_KIND,
      schemaVersion: HUB_ELYSIA_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runElysiaSmoke();
