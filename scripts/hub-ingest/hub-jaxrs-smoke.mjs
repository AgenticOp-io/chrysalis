#!/usr/bin/env node
/**
 * Smoke: hub-gold-jaxrs JAX-RS dialect → WebIR hole-free (20 routes).
...
export const HUB_JAXRS_SMOKE_KIND = "chrysalis.hub.jaxrs-smoke";
export const HUB_JAXRS_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-jaxrs");
 ...
 */
export async function runJaxrsSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const resourceFile = join(root, "src", "HubResource.java");
  if (!existsSync(resourceFile)) {
    return {
      kind: HUB_JAXRS_SMOKE_KIND,
      schemaVersion: HUB_JAXRS_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-hub-resource-java",
      routeCount: 0,
      holeCount: null,
  ...
  return {
    ...
    routeCount,
    holeCount,
    jaxrsRouteCount: liftReport.astRouteCount ?? null,
...
async function main() {
  const report = await runJaxrsSmoke();
