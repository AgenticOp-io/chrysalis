#!/usr/bin/env node
/**
 * Bootstrap hub-flagship-go: Gin 20-route origin → WebIR → CWL (D6448-ST cwl-api).
 * Mirror of hub-flagship-express / hub-flagship-python; no invented product UI (D6447).
 */
...
export const HUB_GO_FLAGSHIP_KIND = "chrysalis.hub.go-flagship";
export const HUB_GO_FLAGSHIP_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-flagship-go");
const liftScript = join(scriptRoot, "scripts/hub-ingest/lift-to-webir.mjs");

/**
 * @param {string} [projectDir]
 */
export async function runGoFlagshipSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const mainGo = join(root, "main.go");
  if (!existsSync(mainGo)) {
    return {
      kind: HUB_GO_FLAGSHIP_KIND,
      schemaVersion: HUB_GO_FLAGSHIP_SCHEMA_VERSION,
      ok: false,
  ...
  const cwlMeta = await exportProjectMigrationCwl(root, { origin: "go" });
  ...
  const { text } = renderCwlRoutes(routes, {
    header: "# Chrysalis Web Language — hub emit from go",
    moduleName: "hub",
  });
  ...
  const verifySeed = join(scriptRoot, "fixtures/ci/hub-flagship-go-verify-for-status/summary.json");
...
async function main() {
  const report = await runGoFlagshipSmoke();
