#!/usr/bin/env node
/**
 * Smoke: hub-gold-roda Roda dialect → WebIR hole-free (20 routes).
 * Does not replace Sinatra hub-flagship-ruby D6448-ST. G10022 / D6484.
 */
...
export const HUB_RODA_SMOKE_KIND = "chrysalis.hub.roda-smoke";
export const HUB_RODA_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-roda");
 ...
 */
export async function runRodaSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "app.rb");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_RODA_SMOKE_KIND,
      schemaVersion: HUB_RODA_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-rb",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runRodaSmoke();
