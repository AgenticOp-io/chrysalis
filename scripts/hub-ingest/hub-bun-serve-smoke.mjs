#!/usr/bin/env node
/**
 * Smoke: hub-gold-bun-serve Bun.serve TS dialect → WebIR hole-free (20 routes).
 * ORIGIN secondary — does not replace Express hub-flagship-express /
 * hub-flagship-typescript D6448-ST. fetch/websocket/plugins stay honest holes.
 */
...
export const HUB_BUN_SERVE_SMOKE_KIND = "chrysalis.hub.bun-serve-smoke";
export const HUB_BUN_SERVE_SMOKE_SCHEMA_VERSION = 1;

const scriptRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..");
const fixture = join(scriptRoot, "fixtures/hub-gold-bun-serve");
 ...
 */
export async function runBunServeSmoke(projectDir = fixture) {
  const root = resolve(projectDir);
  const appFile = join(root, "src", "app.ts");
  if (!existsSync(appFile)) {
    return {
      kind: HUB_BUN_SERVE_SMOKE_KIND,
      schemaVersion: HUB_BUN_SERVE_SMOKE_SCHEMA_VERSION,
      ok: false,
      skip: "missing-app-ts",
      routeCount: 0,
      holeCount: null,
...
async function main() {
  const report = await runBunServeSmoke();
