#!/usr/bin/env node
/** Oracle capture for lift-helper-sql-param-inline (B5.5 v5). */
...
try {
  ...
  await fetch(`${base}/mu`);
  await fetch(`${base}/nu`);
  console.log("[drive-lift-helper-sql-param-inline] exercised /alpha, /beta, /gamma, /epsilon, /delta, /zeta, /iota, /kappa, /lambda, /mu, and /nu");
} finally {
  await handle.stop();
}
