#!/usr/bin/env node
/** Oracle capture for lift-helper-sql-same-twin (B5.3 v5). */
...
try {
  ...
  console.log("[drive-lift-helper-sql-same-twin] php not on PATH — skipping");
  process.exit(0);
}
...
try {
  ...
  console.log("[drive-lift-helper-sql-same-twin] exercised /alpha and /beta");
} finally {
  await handle.stop();
}

const corpus = readCorpus({ root: traceDir });
console.log(`[drive-lift-helper-sql-same-twin] corpus: ${corpus.traces.length} traces`);
