...
if [[ "${CHRYSALIS_GCE_INTELLIGENCE_SHORTHAND:-1}" != "0" ]]; then
  ...
  log "phase: LLM-assisted convert close (G8830, CPU only)"
  run_phase llm-assisted-convert-close env CHRYSALIS_POC_SKIP_BUILD=1 CHRYSALIS_WEB_LLM_TRAJECTORY=1 pnpm run hub:llm-assisted-convert-close-smoke
  ...
  skip_phase llm-assisted-convert-close
