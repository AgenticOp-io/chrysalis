 ...
 *
 * Protected paths (parity HTML + client redirects) are skipped during markup
 * patch; client redirects are re-applied last so load-bind / force-settle
 * cannot leave dead-end spinner shells (G9830 / D6404).
 */
import { existsSync, readFileSync } from "node:fs";
import { dirname, join, resolve } from "node:path";
import { fileURLToPath, pathToFileURL } from "node:url";
import { routesPath } from "./cwl-apply-surfaces.mjs";
import { WISP_BULK_LIFT_SKIP_PATHS } from "./cwl-bulk-svelte-lift.mjs";
import {
...
/** Paths that must not be overwritten by package UI markup lift. */
export function wispPackageUiLiftSkipPaths() {
  return new Set([
    ...WISP_BULK_LIFT_SKIP_PATHS,
    ...WISP_CLIENT_REDIRECT_ROUTES.map((r) => r.path),
    ...WISP_FORCE_MODULE_DEMO_PATHS,
  ]);
}
...
export async function applyWispPackageUiLift(opts = {}) {
  ...
  const skipPaths = wispPackageUiLiftSkipPaths();
  const ingest = await loadIngest();
  const convert = ingest.convertSiteProjectUi({
    projectDir: wispRoot,
    cwlPaths: [path],
    skipHttpPaths: [...skipPaths],
  });

  // Redirects last — same ordering as full-build / deploy bundle (D6404).
  ...
  return {
    kind: WISP_PACKAGE_UI_LIFT_KIND,
    schemaVersion: WISP_PACKAGE_UI_LIFT_SCHEMA_VERSION,
    ok: convert.ok === true && redirects.ok === true,
    wispRoot,
    routesPath: path,
    ...
    skipPaths: [...skipPaths],
    clientRedirects: {
      ok: redirects.ok === true,
      patched: redirects.patched ?? [],
      skip: redirects.skip ?? null,
    },
