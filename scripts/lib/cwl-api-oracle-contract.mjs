...
export function patchApiProxyHandlerBlock(text, method, path, block) {
  ...
  if (start < 0) {
    // Insert new route before first @page (or append) so ArcGIS/grind paths can ship (G9947).
    const pageIdx = text.search(/\n@page /);
    if (pageIdx >= 0) {
      const next = `${text.slice(0, pageIdx).trimEnd()}\n\n${block}\n${text.slice(pageIdx)}`;
      return { ok: true, text: next.endsWith("\n") ? next : `${next}\n`, inserted: true };
    }
    const next = `${text.trimEnd()}\n\n${block}\n`;
    return { ok: true, text: next, inserted: true };
  }
