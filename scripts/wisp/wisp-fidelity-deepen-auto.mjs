 ...
 *   node scripts/wisp/wisp-fidelity-deepen-cli.mjs --until-exhausted
 *
...
export function seedExactPathsFromHistory(state) {
  ...
  const batchesDir = join(scriptRoot, "scripts/wisp/wisp-fidelity-deepen-batches");
...
export function updateCatalogAfterAutoRound(opts) {
  ...
  catalog.desk.autoExhaust = {
    protocol: "scripts/wisp/wisp-fidelity-deepen-auto.mjs",
    stopAfterNoImprovement: STOP_AFTER_NO_IMPROVEMENT,
    decision: "D6445",
    lastBatchId: batchId,
...
export function writeAutoBatchModule(batchId, passStart, targets) {
  const file = join(scriptRoot, `scripts/wisp/wisp-fidelity-deepen-batches/${batchId}.mjs`);
...
export async function runAutoRound(opts = {}) {
  ...
  const modPath = join(scriptRoot, `scripts/wisp/wisp-fidelity-deepen-batches/${batchId}.mjs`);
