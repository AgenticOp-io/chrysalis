 ...
 *
 *   node scripts/wisp/wisp-convert-restart.mjs
 *   node scripts/wisp/wisp-convert-restart.mjs --skip-deploy
 *   node scripts/wisp/wisp-convert-restart.mjs --skip-deepen
 */
...
export async function runWispConvertRestart(opts = {}) {
  ...
  {
    const r = runNode("scripts/wisp/wisp-fidelity-deepen-cli.mjs", ["--external-deps"]);
    steps.push({
      step: "external-deps",
      ok: r.ok || r.status === 0,
  ...
  if (!opts.skipDeepen) {
    const r = runNode("scripts/wisp/wisp-fidelity-deepen-cli.mjs", [
      "--until-exhausted",
      "--reset-streak",
    ]);
    steps.push({
