...
| `chrysalis ingest … --ingest-progress-file` (also **`emit`** ingest phase, **`status --project`**) | **`kind`: `"chrysalis.ingest.progress"`**, **`schemaVersion`**: `0` | Per-route forensic progress (does not skip ingest). Validate with **`@chrysalis/ingest`** **`parseIngestProgressJson`** / **`readIngestProgressFile`**. Fixture: **`fixtures/ci/ingest-progress-v0-smoke.json`**. **DESIGN D277**, **D278**. |
