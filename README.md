...
**Current workspace version: 2.0.1** (see [`CHANGELOG.md`](./CHANGELOG.md); tag **`v2.0.1`** on [GitHub Releases](https://github.com/AgenticOp-io/chrysalis/releases) when published). Install from a tarball: [`docs/INSTALLATION.md`](./docs/INSTALLATION.md#installing-from-a-release-tarball). The repo remains a **fast-moving monorepo**: milestones through **6A** (scoped) and **V2-M1–V2-M6** (scale-out) are complete per [`ROADMAP.md`](./ROADMAP.md); cross-cutting parser, oracle, verify depth, optional repair work, and roadmap **Remaining** slices continue on `main`.
...
| `node scripts/aggregate-chimera-operator-snapshots.mjs` (stdin or NDJSON files) | **`kind`: `"chrysalis.chimera.operator-snapshot.batch"`**, **`schemaVersion`**: `1` | Merges **`chrysalis.chimera.operator-snapshot`** lines (for example from **`chrysalis deploy --operator-metrics-ndjson`**). Fixture: **`fixtures/ci/chimera-operator-snapshot-batch-v1-smoke.json`**. **DESIGN D259**, micro-slices **D260–D269**. |
| `chrysalis emit … --emit-handler-fingerprints` | **`kind`: `"chrysalis.emit.handlerFingerprints"`**, **`schemaVersion`**: `1` | Writes **`chrysalis.emit-handler-fingerprints.json`** at emit **`--out`**. **DESIGN D259**. |
| `chrysalis emit … --emit-runtime-facade` | Emitted **`src/chrysalis-runtime-facade.ts`** (re-export **`./runtime.js`**); API **`emitStrategy.runtimeFacadeModule`**. | Optional stable import surface for PHP lowering shims; handlers/barrel route imports through the facade when set. **DESIGN D272**. |
| `chrysalis emit … --emit-shared-runtime-imports` | Emitted **`src/chrysalis-runtime-imports.ts`** (aggregated **`export { … }`** from **`./runtime.js`** or facade); API **`emitStrategy.emitSharedRuntimeImports`**. | Non-barrel handlers import lowering helpers through one module; not combinable with **`--emit-handler-import-barrel`**. **DESIGN D281**. |
| `chrysalis emit … --emit-dedupe-identical-handler-bodies` | Emitted **`src/chrysalis-deduped/<exportId>.ts`** plus thin per-route handlers; API **`emitStrategy.emitDedupeIdenticalHandlerBodies`**. | Text-identical lowered bodies after **`emitHandlerBody`** share one module; WebIR unchanged. **DESIGN D282**. |
| `node scripts/aggregate-verify-summaries.mjs` (NDJSON or one summary JSON per file) | **`kind`: `"chrysalis.verify.summary.batch"`**, **`schemaVersion`**: `1` | Merges **`chrysalis verify --json-summary`** lines for fleet dashboards. Fixture: **`fixtures/ci/verify-summary-batch-v1-smoke.json`**. **DESIGN D271**. |
| `chrysalis ingest … --ingest-progress-file` (also **`emit`** ingest phase, **`verify --project`**, **`repair`**, **`insight`**, **`status --project`**) | **`kind`: `"chrysalis.ingest.progress"`**, **`schemaVersion`**: `0` | Per-route forensic progress (does not skip ingest). Validate with **`@chrysalis/ingest`** **`parseIngestProgressJson`** / **`readIngestProgressFile`**. Fixture: **`fixtures/ci/ingest-progress-v0-smoke.json`**. **DESIGN D277**, **D278**, **D280**. |
| **`chrysalis ingest` / `emit` / … `--ingest-dedupe-structural-subgraphs`** | (no separate JSON artifact) | Optional post-lowering **`dedupeStructuralSubgraphsInModule`** in **`@chrysalis/webir`**; same structural key as **`mergeWebIrModules`** (**D247**). On **`fixtures/tiny-blog`**, monolithic dedupe matches **K=2** merged shard **node** count (**DESIGN D283**). |
