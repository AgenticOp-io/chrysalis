...
- **Tests — D282 dedupe combinations:** **`emit-hono`** / **`emit-fastify`** **`emit.test.ts`** cover dedupe with **`emitRoutePathConstants`**, **`runtimeFacadeModule`**, and **`emitHandlerFingerprints`**; **`packages/cli/tests/emit-dedupe-identical-handler-bodies-cli.test.ts`** covers Hono **`emit`** with **`--emit-route-path-constants`**; **`packages/ingest/tests/tiny-blog.test.ts`** ingests **`fixtures/dedupe-identical-handlers/`**.
