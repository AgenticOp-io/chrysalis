...
- **Within-module structural subgraph dedupe (DESIGN D283):** **`dedupeStructuralSubgraphsInModule`** in **`@chrysalis/webir`** (same structural key as **`mergeWebIrModules`** / **D247**); **`IngestOptions.dedupeStructuralSubgraphs`**; CLI **`--ingest-dedupe-structural-subgraphs`** on **`ingest`**, **`emit`**, **`status --project`**, **`verify --project`**, **`repair`**, **`insight`**, **`rewrite`**. Vitest **`packages/webir/tests/merge-modules.test.ts`**, **`packages/ingest/tests/tiny-blog.test.ts`**, **`packages/cli/tests/cli-help-scaleout.test.ts`**. **`docs/OPERATIONS.md`**, **`ROADMAP.md`** V2-M4 *Remaining*; package READMEs (**`webir`**, **`ingest`**, **`cli`**).
