# Chrysalis - Roadmap archive (completed history)

> Finished work moved out of [`ROADMAP.md`](./ROADMAP.md) to keep the active
> plan readable. This file is the durable log of shipped slices and closed
> milestones. The authoritative design + decision log stays in
...
## Shipped slices (checkbox backlog + G-series)
...
- [x] **G2286 — lib FunctionDecl attributes on WebIR calls** — **`collectLibraryFunctionAttributes`**; fixture **`lift-helper-attr-lib`**. (**DESIGN D2286**)
- [x] **G2287 — emit phpAttrs provenance suffix + hub semantic smoke v3** — TS comment suffix on **`data.call`**; smoke covers **`sql-ws-twin`** / **`sql-same-twin`**. (**DESIGN D2287**)
- [x] **G2288 — class method attributes + simulate/footprint phpAttributes + B5.3 v4 simulate** — parser hoists **`attrGroups`** on static/`__invoke` methods; **`SimResponse.phpAttributedCalls`**; **`phpAttributedCallCount`** in oracle footprint; hub **`lift-helper-attr-lib`** smoke; **`lift-helper-sql-same-twin`** simulate equivalence. (**DESIGN D2288**)
- [x] **G2289 — class static attr on calls + hub attr smoke in completion** — **`resolveFunctionAttributes`** suffix match for **`Class::method`**; fixture **`lift-helper-attr-class`**; hub completion wires **`irHelperLiftingAttr`**. (**DESIGN D2289**)
- [x] **G2290 — attr hub smoke v2 + ingest simulate metadata e2e** — hub attr smoke covers **`attr-lib`** + **`attr-class`**; Vitest **`lift-helper-attr-lib-ingest-simulate`**. (**DESIGN D2290**)
- [x] **G2291 — B5.3 v5 oracle twin verify for sql-same-twin** — drive + body/SQL parity gate; semantic alias check; **`pnpm run verify:lift-helper-sql-same-twin-oracle`**. (**DESIGN D2291**)
- [x] **G2292 — B5.4 SQL keyword case normalization for semantic lift** — **`normalizeSqlLiteralForHelperLift`** keyword uppercasing; fixture **`lift-helper-sql-case-twin`**; hub semantic smoke **v4**. (**DESIGN D2292**)
- [x] **G2293 — Hub oracle twin gate in completion batch** — **`hub-ir-helper-lifting-oracle-twin-smoke.mjs`** wraps sql-same-twin verify; **`irHelperLiftingOracleTwin`** in hub completion / capability matrix. (**DESIGN D2293**)
- [x] **G2294 — Ingest inline zero-arg lib helpers (direct `return` query)** — **`tryInlineLibHelperCall`** replaces opaque **`data.call`** with helper body; Vitest **`lift-helper-sql-same-twin-inline`**. (**DESIGN D2294**)
- [x] **G2295 — B5.4 v2 oracle twin verify for sql-case-twin** — drive + body/SQL parity + semantic alias; **`verify-lift-helper-sql-case-twin-oracle.mjs`**. (**DESIGN D2295**)
- [x] **G2296 — Hub oracle twin smoke v2 (same-twin + case-twin)** — **`hub-ir-helper-lifting-oracle-twin-smoke.mjs`** schema **v2** runs both oracle gates. (**DESIGN D2296**)
- [x] **G2297 — B5.5 v2 emit HTTP replay verify for sql-same-twin** — ingest inline → emit Hono → **`replayCorpus`** at 100%; **`verify-lift-helper-sql-same-twin-replay.mjs`**. (**DESIGN D2297**)
- [x] **G2298 — B5.4 v3 quote-aware SQL keyword normalization** — **`normalizeSqlLiteralForHelperLift`** skips string literals; Vitest guard. (**DESIGN D2298**)
- [x] **G2299 — B5.5 v3 parametric + multi-stmt lib helper inlining** — **`HelperBodyEntry`** + clone/substitute at call sites; fixture **`lift-helper-sql-param-inline`**. (**DESIGN D2299**)
- [x] **G2300 — sql-case-twin emit HTTP replay verify** — **`verify-lift-helper-sql-case-twin-replay.mjs`** shared replay core. (**DESIGN D2300**)
- [x] **G2301 — Lane A nikic union type hints** — **`typeHint`** maps **`UnionType`** / **`NullableType`** to pipe syntax. (**DESIGN D2301**)
- [x] **G2302 — Lane A readonly class properties (`ClassDecl`)** — **`PhpClassDecl`** metadata; parity page **`readonly_class.php`**. (**DESIGN D2302**)
- [x] **G2303 — Hub emit replay twin gate in completion batch** — **`hub-ir-helper-lifting-replay-twin-smoke.mjs`** runs sql-same/case replay verify; **`irHelperLiftingReplayTwin`** in hub completion. (**DESIGN D2303**)
- [x] **G2304 — Hub semantic smoke v5 (`sql-param-inline`)** — **`lift-helper-sql-param-inline`** in semantic lift batch. (**DESIGN D2304**)
- [x] **G2305 — Parser parity ingest for `readonly_class.php`** — zero-hole route ingest Vitest. (**DESIGN D2305**)
- [x] **G2306 — sql-param-inline emit HTTP replay verify** — **`verify-lift-helper-sql-param-inline-replay.mjs`**; hub replay twin smoke **v2**. (**DESIGN D2306**)
- [x] **G2307 — B5.4 v4 quote-aware SQL normalization edge tests** — double-quoted literals, escaped quotes; Vitest guards. (**DESIGN D2307**)
- [x] **G2308 — B5.5 v5 multi-assign lib helper inlining** — N assign-then-return chains in **`tryExtractInlineQuery`**; **`chrysalis_sql_param_chain`** in param-inline fixture. (**DESIGN D2308**)
- [x] **G2309 — Lane A constructor property promotion (`ClassDecl`)** — promoted **`__construct`** params map to **`PhpClassProperty`**; parity page **`constructor_promotion.php`**. (**DESIGN D2309**)
- [x] **G2310 — Lane A union type parity page** — **`union_type.php`** glayzzle ≡ nikic; route + ingest coverage. (**DESIGN D2310**)
- [x] **G2311 — Lane A intersection type hints** — **`IntersectionType`** / glayzzle **`intersectiontype`** → **`A&B`**; parity page **`intersection_type.php`**. (**DESIGN D2311**)
- [x] **G2312 — B5.5 inline negative control** — **`chrysalis_sql_param_noinline`** (non-chain assign) stays a call; **`/delta`** route. (**DESIGN D2312**)
- [x] **G2313 — Lane A `readonly class` modifier on `ClassDecl`** — **`readonly: true`** metadata; parity page **`readonly_class_keyword.php`**. (**DESIGN D2313**)
- [x] **G2314 — B5.4 v5 backtick SQL identifier quoting** — **`normalizeSqlLiteralForHelperLift`** skips keywords inside backticks. (**DESIGN D2314**)
- [x] **G2315 — B5.4 v6 SQL comment quoting** — line/block comments skip keyword uppercasing; Vitest guards. (**DESIGN D2315**)
- [x] **G2316 — B5.5 v6 effect-free prelude stmt skip for inline** — assign chain + skipped pure expr stmts; **`sql_param_prelude`** + **`/epsilon`**. (**DESIGN D2316**)
- [x] **G2317 — Lane A `never` return type parity page** — **`never_type.php`** glayzzle ≡ nikic. (**DESIGN D2317**)
- [x] **G2318 — B5.5 emit lib-helper module** — **`meta.helperBodies`** + **`src/lib-helpers.ts`** for non-inlinable helpers; **`/delta`** emit replay. (**DESIGN D2318**)
- [x] **G2319 — B5.5 effectful prelude negative control** — **`sql_param_sideeffect`** + **`/zeta`**. (**DESIGN D2319**)
- [x] **G2320 — Lane A enum method hoisting** — **`enum_methods.php`**; **`Color::label`** / **`Color::default`**. (**DESIGN D2320**)
- [x] **G2321 — B5.5 emit lib-helpers on Fastify** — mirror Hono **`src/lib-helpers.ts`** wiring; Vitest on **`lift-helper-sql-param-inline`**. (**DESIGN D2321**)
- [x] **G2322 — Lane A nikic `self::` StaticFetch parity** — **`classFqnForStaticLike`** maps **`self`/`static`/`parent`** to empty **`className`** (glayzzle-aligned). (**DESIGN D2322**)
- [x] **G2323 — Lane A `mixed` type parity page** — **`mixed_type.php`** glayzzle ≡ nikic. (**DESIGN D2323**)
- [x] **G2324 — B5.5 prelude inline guard Vitest** — **`tryExtractInlineQuery`** rejects effectful prelude; allows **`strlen`** skip. (**DESIGN D2324**)
- [x] **G2325 — B5.5 literal-RHS assign inlining** — **`$flag = 1`** chain in **`tryExtractInlineQuery`**; **`/iota`** positive control. (**DESIGN D2325**)
...
- [x] **G2401 — Parser-parity-probe full-fixture hole budget** — manifest ingest pins **5** routes; Vitest **`parser-parity-probe-ingest.test.ts`**. (**DESIGN D2401**)
- [x] **G2402 — Hub verify-gaps program closure** — STRATEGIC-PLAN months **26–30** + post110 Phase **B** closed. (**DESIGN D2402**)
- [x] **G2403 — Phase 0 + post-2.0 packaging closure** — capability matrix, commercial/multi-runtime CLI ROADMAP rows. (**DESIGN D2403**)
- [x] **G2404 — V2-M4 helper-lift archive note** — non-B5 semantic lift → maintenance backlog. (**DESIGN D2404**)
- [x] **G2405 — Parser-parity-probe zero-hole manifest ingest** — **`NullsafePropertyFetch`**, **`NullsafeMethodCall`**, **`ThrowExpr`** AST + ingest lowering; instance **`(new X())->method()`** → **`__method_call`**; CI **`--max-holes 0`**. (**DESIGN D2405**)
- [x] **G2406 — SQLite3 `->query` receiver tracking** — **`new SQLite3`** alias tracking; **`GET /widgets/sqlite3-query`** on **mysqli-probe**; CI **`--max-holes 0`**. (**DESIGN D2406**)
- [x] **G2407–G2411 — Queue 111 Phase C resume** — plan amendment; **`runPost111CompositeGate`**; batch **v111**; hub-completion schema **184**. (**DESIGN D2407**)
- [x] **G2417–G2421 — Queue 112 template/budget depth** — **`runSvelteEachPartialLiftGate`**; **`runPost112CompositeGate`**; batch **v112**; schema **185**. (**DESIGN D2417**)
...
- [x] **G2557–G2561 — Queue 126 tri-origin verify-gaps** — **`runPost126CompositeGate`**; batch **v126**; schema **199**. (**DESIGN D2557**)
- [x] **G2567–G2571 — Queue 127 verify-gaps ingest closure** — **`runPost127CompositeGate`**; batch **v127**; schema **200**. (**DESIGN D2567**)
- [x] **G2577–G2581 — Queue 128 auth-probe reingest HTTP** — **`runPost128CompositeGate`**; batch **v128**; schema **201**. (**DESIGN D2577**)
- [x] **G2587–G2591 — Queue 129 IR helper lifting** — **`runPost129CompositeGate`**; batch **v129**; schema **202**. (**DESIGN D2587**)
- [x] **G2597–G2601 — Queue 130 post-90 verify-gaps composite** — **`runPost130CompositeGate`**; batch **v130**; schema **203**. (**DESIGN D2597**)
- [x] **G2607–G2611 — Queue 131 session + runtime + emit** — **`runPost131CompositeGate`**; batch **v131**; schema **204**. (**DESIGN D2607**)
- [x] **G2617–G2621 — Queue 132 delivery + flagship** — **`runPost132CompositeGate`**; batch **v132**; schema **205**. (**DESIGN D2617**)
- [x] **G2627–G2631 — Queue 133 post-60 authoring** — **`runPost133CompositeGate`**; batch **v133**; schema **206**. (**DESIGN D2627**)
- [x] **G2637–G2641 — Queue 134 post-50 HTTP gaps** — **`runPost134CompositeGate`**; batch **v134**; schema **207**. (**DESIGN D2637**)
- [x] **G2647–G2651 — Queue 135 post-40 chimera delivery** — **`runPost135CompositeGate`**; batch **v135**; schema **208**. (**DESIGN D2647**)
- [x] **G2657–G2661 — Queue 136 post-30 runtime parity** — **`runPost136CompositeGate`**; batch **v136**; schema **209**. (**DESIGN D2657**)
- [x] **G2667–G2671 — Queue 137 post-60/50 stack** — **`runPost137CompositeGate`**; batch **v137**; schema **210**. (**DESIGN D2667**)
- [x] **G2677–G2681 — Queue 138 post-61 preview dev** — **`runPost138CompositeGate`**; batch **v138**; schema **211**. (**DESIGN D2677**)
- [x] **G2687–G2691 — Queue 139 post-62 CWL parity** — **`runPost139CompositeGate`**; batch **v139**; schema **212**. (**DESIGN D2687**)
- [x] **G2697–G2701 — Queue 140 month-2 mega** — **`runPost140CompositeGate`**; batch **v140**; schema **213**. (**DESIGN D2697**)
- [x] **G2707–G2711 — Queue 141 post-73/74/75 stack** — **`runPost141CompositeGate`**; batch **v141**; schema **214**. (**DESIGN D2707**)
- [x] **G2717–G2721 — Queue 142 post-76/77 search export** — **`runPost142CompositeGate`**; batch **v142**; schema **215**. (**DESIGN D2717**)
- [x] **G2727–G2731 — Queue 143 post-78/79 deep export** — **`runPost143CompositeGate`**; batch **v143**; schema **216**. (**DESIGN D2727**)
- [x] **G2737–G2741 — Queue 144 month-23 graduation lock** — **`runPost144CompositeGate`**; batch **v144**; schema **217**. (**DESIGN D2737**)
- [x] **G2747–G2751 — Queue 145 Phase D graduation lock** — **`runPost145CompositeGate`**; batch **v145**; schema **218**. (**DESIGN D2747**)
- [x] **G2757–G2951 — Queues 146–165 post-63..82 composite replay** — **`runPost146CompositeGate`** through **`runPost165CompositeGate`**; batches **v146–v165**; schema **219–238**. (**DESIGN D2757**)
- [x] **G2957–G3051 — Queues 166–175 post-83..102 replay (Phase E)** — **`runPost166CompositeGate`** through **`runPost175CompositeGate`**; batches **v166–v175**; schema **239–248**. (**DESIGN D2957**)
- [x] **G3057–G3151 — Queues 176–185 Phase F hub-bridge replay** — **`runPost176CompositeGate`** through **`runPost185CompositeGate`**; batches **v176–v185**; schema **249–258**. (**DESIGN D3057**)
- [x] **G3157–G3251 — Queues 186–195 Phase G Phase C replay** — **`runPost186CompositeGate`** through **`runPost195CompositeGate`**; batches **v186–v195**; schema **259–268**. (**DESIGN D3157**)
- [x] **G3257–G3351 — Queues 196–205 Phase H post-124..133 replay** — **`runPost196CompositeGate`** through **`runPost205CompositeGate`**; batches **v196–v205**; schema **269–278**. (**DESIGN D3257**)
- [x] **G3357–G3451 — Queues 206–215 Phase I post-134..143 replay** — **`runPost206CompositeGate`** through **`runPost215CompositeGate`**; batches **v206–v215**; schema **279–288**. (**DESIGN D3357**)
- [x] **G3457–G3551 — Queues 216–225 Phase J post-144..153 replay** — **`runPost216CompositeGate`** through **`runPost225CompositeGate`**; batches **v216–v225**; schema **289–298**. (**DESIGN D3457**)
- [x] **G3557–G3751 — Queues 226–245 Phase K post-154..173 replay** — **`runPost226CompositeGate`** through **`runPost245CompositeGate`**; batches **v226–v245**; schema **299–318**. (**DESIGN D3557**)
- [x] **G3757–G3951 — Queues 246–265 Phase L post-174..193 replay** — **`runPost246CompositeGate`** through **`runPost265CompositeGate`**; batches **v246–v265**; schema **319–338**. (**DESIGN D3757**)
- [x] **G3957–G4151 — Queues 266–285 Phase M post-194..213 replay** — **`runPost266CompositeGate`** through **`runPost285CompositeGate`**; batches **v266–v285**; schema **339–358**. (**DESIGN D3957**)
- [x] **G4157–G4351 — Queues 286–305 Phase N post-214..233 replay** — **`runPost286CompositeGate`** through **`runPost305CompositeGate`**; batches **v286–v305**; schema **359–378**. (**DESIGN D4157**)
- [x] **G4357–G4551 — Queues 306–325 Phase O post-234..253 replay** — **`runPost306CompositeGate`** through **`runPost325CompositeGate`**; batches **v306–v325**; schema **379–398**. (**DESIGN D4357**)
- [x] **G4557–G4751 — Queues 326–345 Phase P post-254..273 replay** — **`runPost326CompositeGate`** through **`runPost345CompositeGate`**; batches **v326–v345**; schema **399–418**. (**DESIGN D4557**)
- [x] **G4757–G4951 — Queues 346–365 Phase Q post-274..293 replay** — **`runPost346CompositeGate`** through **`runPost365CompositeGate`**; batches **v346–v365**; schema **419–438**. (**DESIGN D4757**)
- [x] **G4957–G5671 — Queues 366–437 Phase R–U post-294..365 replay (full-stack ladder complete)** — **`runPost366CompositeGate`** through **`runPost437CompositeGate`**; batches **v366–v437**; schema **439–510**. (**DESIGN D4957**)
