...
- **2026-04-25 — D115** **Composer template mixed aggregate snapshot (`/chrysalis-items-snapshot`).**
  Adds **`items_snapshot_show.php`** with a single query
  **`SELECT COUNT(*) AS c, MIN(id) AS min_id, MAX(id) AS max_id, SUM(id) AS sum_id FROM items`**
  via **`query_one`**, returning
  **`{"itemsSnapshot":{"count":3,"minId":1,"maxId":3,"sumId":6}}`** on seeded ids **1..3**.
  Manifest, Laravel stub routes, **`verify-flagship-laravel-full`** (two **`GET`** captures),
  ingest + dual-emit parity tests, and README counts move to **forty-seven** template handlers.
  Same incremental oracle scope as **D86–D114**, but with a higher-signal mixed-aggregate shape.
