...
ingest + dual-emit parity tests, and README counts move to **fifty-one** template handlers (D190 adds **`chrysalis-auth-probe`**).
...
- **2026-04-27 — D190** **Flagship oracle probes for `Gate` + Sanctum/OAuth-shaped callees.**
  ...
  **Ingest** lowers these calls to deterministic WebIR (`Gate::{allows,denies}` → string equality on the
  ...
  the auth-probe handler builds JSON via **string concat** (not `json_encode`) so emit does not need a
  JSON primitive for this fixture.
