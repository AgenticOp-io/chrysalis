...
- **2026-05-02 — D279** **Operator JSON index — ingest progress.** Root **`README.md`** *Machine-readable JSON* table adds **`chrysalis.ingest.progress`** **`schemaVersion` 0** (source: **`chrysalis ingest|emit|status`** **`--ingest-progress-file`**). **`docs/README.md`** Operations guide row cites **D278** parse helpers beside **D277**. **DESIGN §3:** documentation only.
