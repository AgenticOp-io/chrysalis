...
├── branding/                   # outbound marks (AgenticOp SVGs; not MIT-licensed as marks)
│   └── agenticop/
├── pnpm-workspace.yaml
├── tsconfig.base.json
├── .gitignore
...
- **2026-05-16 — D299** **WPTP OpenAPI → Hono bronze compose path.** **`theorem6/wptp-ir`** export synthesizes stub **handler** subgraphs for contract-only routes; **`exportIrToWebIrBundleV0`** feeds **`scripts/emit-webir-bundle-hono.mjs`** and **`@chrysalis/webir` `moduleFromGoldenSnapshot`**. Matrix path **`openapi-ir-hono`** (requires **`CHRYSALIS_ROOT`**) and matrix row **`openapi-to-hono-ts`** move to **bronze supported**. **DESIGN §3:** emit still runs through existing **`@chrysalis/emit-hono`** only.
