...
- **2026-06-19 — D6204** **Phase 14 HSS operator; ACS excluded from CWL depth.** User-amended strategy: **`acs-hss-server` / `hss.wisptools.io`** hosts **GenieACS + MongoDB + backend-services** (operator infra — built for GenieACS, Mongo modernized in place). **Phase 14 active queue:** HSS site / chimera deploy only. **ACS / GenieACS / TR-069 permanently excluded** from CWL language goals — no runtime special cases, no ACS widget lowering, no backend conversion. Phase 13 M4 ACS `@page` shells remain **fixture regression only**. **DESIGN §3:** vendor stack catalogued (`hub-cwl:upstream-proxy`); language must generalize.
