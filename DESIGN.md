...
- **2026-05-26 — D339** **Hub gold depth + path explorer (G34).** **`hub-gold-verify`** supports **CWL** emit and **round-trip** re-lift; new fixtures **`hub-gold-js-structured`**, **`hub-gold-python-structured`**, **`hub-gold-ts-literal`**; **javascript/typescript → cwl** promoted to **gold** when structural gates pass; **`lift-to-webir`** ignores **`generated/`** and **`.chrysalis/`** during walks. Operator **Path explorer** (`#/paths`) surfaces synthesis + per-pair path knowledge. Python→CWL remains **silver** until Flask route lift matches literal fixtures. **DESIGN §3** unchanged.
