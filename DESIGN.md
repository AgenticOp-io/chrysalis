...
- **2026-07-17 — D6448-ST** **Complete conversion success template (cross-language).** Operator: zero holes via terminal settle is not the reusable success bar. **ST green** requires evidence-only hole zero (`forceSettleUsed: false`), vendor islands (SharedMap → coverage-map ArcGIS iframe — not empty shell / not Bing), `data-*-page` boot, closed overlays (`hidden` + CSS), idle CTA labels (`isSigningIn` else branch), signed-in origin compare. **Ship:** [`docs/COMPLETE-CONVERSION-SUCCESS-TEMPLATE.md`](docs/COMPLETE-CONVERSION-SUCCESS-TEMPLATE.md); checklist `chrysalis.complete-conversion.checklist.v1`; `hub:complete-conversion-prove`; SharedMap iframe lift; modules `[hidden]` CSS; login busy/idle if fix. Extends **D6448**. **DESIGN §3:** provenance; holes over invention; no silent drift.

- **2026-07-17 — D6448** **Complete conversion protocol — fix holes during convert (all languages).** Operator: conversion must run an honest hole-close loop until **zero** `data-cwl-hole` (or fail incomplete with residual ledger) — not stop after one convert-all pass. **Phases:** corpus → structural lift → honest close loop (re-lift → bind `forceSettle:false` → golden hydrate → UI-toggle stamp) → census → export/CSS/islands → API deepen → sign-in prove vs origin. **Complete iff** hole total === 0 without force-settle; vendor islands may remain as islands, not stripped holes. **Stop incomplete:** 3× no hole reduction → write residuals + convert gate **fails** unless `--allow-incomplete`. **Refuse:** `hub:wisp-deep-lift-all-holes` / force-settle as completeness. **Ship:** [`docs/COMPLETE-CONVERSION-PROTOCOL.md`](docs/COMPLETE-CONVERSION-PROTOCOL.md); `scripts/wisp/wisp-complete-conversion-protocol.mjs`; `pnpm run hub:complete-conversion`; wired into `hub:wisp-convert-restart`. Extends **D6447**. **DESIGN §3:** holes over invention; provenance; no silent drift.

- **2026-07-17 — D6447** **True conversion only — never demo-only code (agent absolute; all languages).** Operator: agents must stay true to the convert process and **never** ship demo façades. Extends **D6442**/**D6443**/**D6444**. **Scope:** every origin→CWL→emit pair (not WISP-only; WISP is POC proof). **Refuse:** hand-built showcase login/dashboard/shells; parity HTML that is not a structural lift of origin; force-settling holes to claim complete; helper chrome/APIs/maps not in source; “good enough for demo” substitutes. **POC hosting** may serve the **converted** artifact only. **After convert:** sign in and test against origin (WISP: `wisptools.io`). **Ship:** `AGENTS.md` absolute law; canon §0/§2A; `.cursor/rules/chrysalis.mdc`; `STRATEGIC-PLAN.md`. **DESIGN §3:** holes over invention; provenance; no silent drift.
- **2026-07-17 — D6445** **External-call + API-key discovery protocol (deepen desk) + auto-exhaust stop.** Operator: before continuing fidelity deepen, identify calls to external servers/hardware and required API keys — if keys do not exist, document them to the user as a whole (do not invent). **Protocol:** scan Module_Manager + `backend-services` for (1) non-first-party `https?://` hosts, (2) `process.env` / `import.meta.env` secret-shaped names, (3) hardware tokens (SNMP/MikroTik/ACS/TR-069), (4) cross-check `.env.example` documented keys; classify local presence without printing values; emit operator briefing. **Auto-continue:** deepen must not wait for repeated “continue” — run `--until-exhausted` / `hub:fidelity-deepen-until-exhausted` which proposes ×10 rounds (static GET → param GET → golden-backed mutations) until **3 consecutive rounds with zero newly green exact method+path probes**. Empty queue alone is **not** a stop (it only increments the streak toward that 3). **Ship:** `scripts/wisp/wisp-external-deps-protocol.mjs`, `scripts/wisp/wisp-fidelity-deepen-auto.mjs`, desk `--external-deps` / `--until-exhausted`, fixtures `chrysalis.wisp-external-deps.v1.json` + `chrysalis.wisp-fidelity-deepen-auto-state.v1.json`. Missing key → honest residual / skip — never invent vendors or credentials (**D6442** / **D6441**). **DESIGN §3:** holes over invention; provenance of secrets as names only.
...
- **2026-07-24 — D6474** **JAX-RS Java secondary dialect smoke (G10012).** After Spring Java D6448-ST, prove secondary JAX-RS-origin lift: class `@Path` prefix join, method `@GET|POST|…` + `@Path`, `{id}` templates, `@PathParam` / `@QueryParam` + `@DefaultValue`, `Map.of` returns, `Response.status().entity().build()`. **Ship:** gold `hub-gold-jaxrs` (single `HubResource`); gate `hub:jaxrs-smoke` (20/20 hole-free); peels in `java-ast-ingest.mjs` + `hub-native-bridge` `parseJavaRoutes` (`@Path(?!Param)`, adjacent annotation blocks, class-scoped dedupe). Not ST — Spring remains Java flagship. Honest holes: CDI, filters, providers, `Application` subclasses. **DESIGN §3 / D6442/D6447:** translate JAX-RS route handlers only.

- **2026-07-24 — D6475** **Starlette Python secondary dialect smoke (G10013).** After Flask Python D6448-ST, prove secondary Starlette-origin lift via existing `@app.route(..., methods=[...])` decorator peel (no new ASGI middleware invent): `{id}` path templates, `request.query_params.get`, Flask-style `(body, status)` tuple returns on POST routes. **Ship:** gold `hub-gold-starlette`; gate `hub:starlette-smoke` (20/20 hole-free). Not ST — Flask remains Python flagship. Honest holes: `Mount`, middleware stack, `Route()`-only tables, `JSONResponse`/`Response` returns, WebSocket. **DESIGN §3 / D6442/D6447:** translate Starlette route handlers only.

- **2026-07-24 — D6477** **NestJS @Headers/@Cookies/@Body field peels (G10015).** Deepen Nest route-surface peel: `@Headers('x')` / `@Header('x')` / `@Cookies('y')` / `@Cookie('y')` / `@Body('field')` → `requestField` via `nestjs-ast-ingest` `PARAM_DECORATORS` + shared JS AST `nestParamBindings`. **Ship:** gold `hub-gold-nestjs` ping (`x-request-id`), flag (`session` cookie), echo (`message` body); gates `hub:nestjs-smoke` + `hub:nestjs-flagship` (20/20 hole-free). DI/guards/pipes/interceptors remain honest holes (**D6447**). **DESIGN §3 / D6442/D6447:** IDENT-safe decorator peels only — no Nest runtime invent.

- **2026-07-24 — D6472** **Go Echo secondary dialect smoke (G10010).** After Gin Go D6448-ST + Chi secondary (D6471), prove secondary Echo-origin lift: `e.GET|POST|PUT|PATCH|DELETE`, `:id` path templates, same-file named `func name(c echo.Context) error`, `c.Param`, `c.QueryParam`, `return c.JSON(status, lit|map[string]interface{}{…})`, `return c.String(status, "…")`. **Ship:** gold `hub-gold-echo`; gate `hub:echo-smoke` (20/20 hole-free); peels in `go-ast-ingest.mjs` via `detectGoWebDialect` (Chi/Gin unchanged). Not ST — Gin remains Go flagship. Honest holes: `e.Use` middleware, `e.Group` deep nesting, `c.Bind`/struct binders, non-literal paths. Catalog: `fixtures/ci/go-secondary-dialect-honest-holes.json`. **DESIGN §3 / D6442/D6447:** translate Echo route handlers only.

- **2026-07-24 — D6473** **Rocket Rust secondary dialect smoke (G10011).** After Actix D6448-ST + Axum secondary (D6456), prove secondary Rocket-origin lift: `#[get|post|put|patch|delete("/path")]`, `.mount("/prefix", routes![handler, …])`, `<id>` / `?<q>` path templates, handler param binds, `Json(serde_json::json!(…))`, `(Status::Created|Accepted, Json(…))`. **Ship:** gold `hub-gold-rocket`; gate `hub:rocket-smoke` (20/20 hole-free); peels in `rust-ast-ingest.mjs` + `pattern-route-parsers.mjs` (`collectRocketMountPrefixes`, `RUST_MACRO_FN_RE`). Not ST — Actix remains Rust flagship. Honest holes: fairings/catchers/guards, `FromForm`/multipart/cookies, managed state, non-literal paths. **DESIGN §3 / D6442/D6447:** translate Rocket route handlers only; no invented fairings/auth.

- **2026-07-24 — D6476** **Hapi multi-method array peel (G10014).** After G9951/G10005, lift `server.route({ method: ['GET','POST'], path, handler })` by expanding the method array to one CWL route per HTTP verb (same handler/path/loc). **Ship:** `peelHapiMethodValues` + `peelHapiRouteObjects` in `javascript-ast-ingest.mjs`; gold `hub-gold-hapi` uses multi-method `/echo` (replaces scalar POST `/echo` + GET `/ready`); gate `hub:hapi-smoke` (20/20 hole-free). Honest holes unchanged: plugins/lifecycle/auth/validate. **DESIGN §3 / D6442/D6447:** translate-only peel; no invented Hapi runtime.

- **2026-07-24 — D6467** **Ktor Kotlin secondary dialect smoke (G10004).** After Spring Kotlin D6448-ST, prove secondary Ktor-origin lift: `routing { get|post|…("/path") { … } }`, `{id}` path templates, `call.parameters` / `call.request.queryParameters`, `call.respond(mapOf(…))` / scalar literals / `HttpStatusCode.Created|Accepted` status tuples, `call.respond(pathRef)` for bound parameters. **Ship:** gold `hub-gold-ktor`; gate `hub:ktor-smoke` (20/20 hole-free). Not ST — Spring remains Kotlin flagship. Honest holes: auth, plugins, nested routing beyond cheap peel, non-literal paths. **DESIGN §3 / D6442/D6447:** translate Ktor route handlers only.

- **2026-07-24 — D6462** **Polka TS secondary dialect smoke (G9958).** Complete thin Node set: `app.get|post|…` + Node `res.writeHead(N)` / `res.end(JSON.stringify(…))` + `req.params|query` (+ `res.statusCode`). **Ship:** gold `hub-gold-polka`; gate `hub:polka-smoke` (20/20 hole-free). Not ST — Express/TS remain flagships; Nest remains only JS secondary ST. Honest holes: `app.use`, named/chained receivers, streaming. Phoenix controller peel skipped (not cheap — see `phoenix-controller-honest-skip.json`). **DESIGN §3 / D6442/D6447:** translate Polka/Node http handlers only.

- **2026-07-24 — D6461** **Restify TS secondary dialect smoke (G9957).** Complete thin Node set after Hapi: `server.get|post|put|patch|del` + `res.send` / Restify `res.send(code, body)` + `req.params|query`. **Ship:** gold `hub-gold-restify`; gate `hub:restify-smoke` (20/20 hole-free); shared JS AST peels `del`→DELETE + two-arg send. Not ST — Express/TS remain flagships; Nest remains only JS secondary ST. Honest holes: `server.pre`/`use`, plugins, lifecycle. **DESIGN §3 / D6442/D6447:** translate Restify handlers only.

- **2026-07-24 — D6460** **Dart/Shelf route-surface D6448-ST (G9956).** Promote `hub-gold-dart-shelf` to cwl-api ST: `hub:dart-flagship` → CWL + verify seed; `hub:complete-conversion-prove:dart` → `stGreen`+`stClosed` (20/20 hole-free). Flutter / Dart Frog / Pipeline / named handlers / mount remain honest holes (**D6447** — not present in gold). Phoenix peel still deferred. **DESIGN §3 / D6442/D6447/D6448-ST:** route surface only.

- **2026-07-24 — D6459** **Elixir Plug.Router route-surface D6448-ST (G9955).** Promote `hub-gold-elixir-plug` to cwl-api ST: `hub:elixir-flagship` → CWL + verify seed; `hub:complete-conversion-prove:elixir` → `stGreen`+`stClosed` (20/20 hole-free). Phoenix LiveView / controller / pipeline remain honest holes (**D6447** — not present in gold; no invent). **DESIGN §3 / D6442/D6447/D6448-ST:** route surface only.

- **2026-07-24 — D6458** **Dart/Shelf foundation smoke (G9954).** First Dart origin language: lift `router.get|post|put|patch|delete('/path', …)` + `Response.ok(jsonEncode(…))` / `Response(status, body: jsonEncode(…))` + `request.url.queryParameters` + `jsonDecode(await request.readAsString())` body peels + `<id>` path params. **Ship:** gold `hub-gold-dart-shelf`; gate `hub:dart-smoke` (20/20 hole-free); catalog `dart` + `.dart`. Not ST — no invented Flutter / Dart Frog / Pipeline runtime (**D6447**). Honest holes: Pipeline middleware, named handlers, `Router.mount`, Flutter/Frog, `router.all`, streaming/hijack. Phoenix controller peel deferred (not cheap without inventing LiveView/controller runtime). **DESIGN §3 / D6442/D6447:** translate Shelf Router only.

- **2026-07-24 — D6457** **Elixir Plug.Router foundation smoke (G9953).** First BEAM origin language: lift `get|post|put|patch|delete "/path" do … end` + `send_resp(conn, status, Jason.encode!(lit|%{…}))` + `conn.params` / `query_params` / `body_params`. **Ship:** gold `hub-gold-elixir-plug`; gate `hub:elixir-smoke` (20/20 hole-free); catalog `elixir` in language catalog + `.ex`/`.exs`. Not ST — no invented Phoenix LiveView/controller runtime (**D6447**). Honest holes: Phoenix `Ctrl,:action`, LiveView, pipeline plugs, nested do/end, `forward`/`match`. Dart closed via D6458. **DESIGN §3 / D6442/D6447:** translate Plug.Router only.

- **2026-07-24 — D6456** **Axum Rust secondary dialect smoke (G9949).** After Actix D6448-ST, prove secondary Axum-origin lift: `.route(path, get|post|…(|| async { … }))` closures (prefer over distant `fn main`), `Json(serde_json::json!(…))`, `(StatusCode::CREATED|ACCEPTED, Json(…))`, Path/Query via path template + `query.get`. **Ship:** gold `hub-gold-axum`; gate `hub:axum-smoke` (20/20 hole-free). Not ST — Actix remains Rust flagship. Honest holes: named `get(handler)`, nest/merge/layers, State/Extension. **DESIGN §3 / D6442/D6447:** translate Axum handlers only. Elixir/Dart still absent.

- **2026-07-24 — D6455** **Nitro deepen: destructure readBody bind (G9947d).** After D6453, lift `const { x } = await readBody(event)` (and renamed `{ x: y }`) to body request fields; header/cookie/`??` already green via nullish peel. Gold `hub-gold-vue-nitro` echo/create use destructure; smoke schema **v4**. Remaining holes: whole-body readBody, rest destructure, header/cookie dumps, non-empty middleware. **DESIGN §3 / D6442/D6447:** translate only; no invented product UI. Elixir/Dart still absent.

- **2026-07-24 — D6454** **Fastify TypeScript origin dialect smoke (G9948).** After Express/TS D6448-ST, prove secondary Fastify-origin lift (distinct from `@chrysalis/emit-fastify` emit target): `fastify.get|post|…` + `reply.send` / `reply.code(n).send` + `req.params`/`req.query` (and `request.*` bags). **Ship:** gold `hub-gold-fastify`; gate `hub:fastify-smoke` (20/20 hole-free). Not ST. Honest holes: plugins/hooks/schema/register. **DESIGN §3 / D6442/D6447:** translate Fastify handlers only. Elixir/Dart still absent.

- **2026-07-24 — D6453** **Nitro deepen: getHeader/getCookie + readBody bindings (G9947c).** After D6452, lift h3 `getHeader` / `getRequestHeader` / `getCookie` and `const body = await readBody(event); body.x` bindings. Gold `hub-gold-vue-nitro` exercises bind/header/cookie; smoke schema **v3**. Remaining holes: whole-body readBody, destructure bind, header/cookie dumps, non-empty middleware. **DESIGN §3 / D6442/D6447:** translate only; no invented product UI. Elixir/Dart still absent.

- **2026-07-24 — D6452** **Nitro deepen: readBody + server/middleware (G9947b).** After D6451 20-route hole-free, lift `(await) readBody(event).field` → body request fields; discover `server/middleware` including nested dirs as global `nitro.middleware` presets (nested ≠ path mount). Gold `hub-gold-vue-nitro` echo/create use readBody; smoke schema **v2** (20 routes + 2 middleware). Catalog holes for bindings / whole-body / non-empty middleware. **DESIGN §3 / D6442/D6447:** translate only; no invented product UI. Elixir/Dart still absent.

- **2026-07-24 — D6451** **Vue/Nuxt Nitro/h3 server route dialect (G9947).** After Express-in-SFC Vue D6448-ST, lift Nuxt `server/api` file routes via `defineEventHandler` / h3 `getRouterParam` / `getQuery` / `setResponseStatus` (same express-depth 20-route surface). **Ship:** `nitro-route-lift.mjs` + JS AST peels; gold `hub-gold-vue-nitro`; gate `hub:vue-nitro-smoke`. Does not replace `hub-flagship-vue` ST. **DESIGN §3 / D6442/D6447:** translate Nitro handlers only; no invented product UI. Elixir/Dart ingest still absent (no gold foundation).
