...
Phase 3, **G139** / **G10002** / **G10031** / **G10054**). It proves Chrysalis can ingest an *external* API contract into
...
| hyphenated cookie / response-header names | skipped (IDENT-safe only — no invent rename) |
...
examples become `openapi:nested-response-body` for the same reason. Cookie params and
response headers are emitted **only** when declared as IDENT-safe with a concrete value —
...
**G10031** / **G10054** cases in `packages/cli/tests/hub-strategic.test.ts`.
