...
- **Pages:** `coalesce_chain.php`, `nested_array.php`, `coalesce_assign.php` (`??=`), `string_interpolation.php` (`"{$name}"`), `nullsafe_property.php` (`?->`), `invokable_controller.php` (class with `__invoke` + static helper + unused instance method — locks the G132 `__invoke` hoist across both providers, G133).
