...
**IR helper lifting B5** — semantic widening for param-name twins (v0), scale-by-2 arithmetic (B5.2), and commutative add (B5.2 v2).
...
- **`chrysalis_comm_alpha($a, $b)`** vs **`chrysalis_comm_beta($m, $n)`** — `return $a + $b` vs `return $n + $m`; aliases under B5.2 v2.
